...
type StoryRunStatus struct {
	...
	// StepOutputs stores the outputs of each completed step.
	// The key is the step ID (from the Story spec).
	// The value is the raw JSON output from the StepRun.
	// This allows subsequent steps to reference the outputs of prior steps.
	// +optional
	// +kubebuilder:pruning:PreserveUnknownFields

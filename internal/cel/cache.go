...
func NewCompilationCache(config *CacheConfig, env *cel.Env) *CompilationCache {
	if config == nil {
...
func (cc *CompilationCache) CompileAndCache(ctx context.Context, expression, expressionType string) (*CachedProgram, error) {
	...
	if cached, exists := cc.cache[key]; exists && !cc.isExpired(cached) {
		...
		cel := logging.NewCELLogger(ctx)
		cel.CacheHit(expression, expressionType)
	...
	cel := logging.NewCELLogger(ctx)
	cel.EvaluationStart(expression, expressionType)
	start := time.Now()
	...
	if issues != nil && issues.Err() != nil {
		...
		cel.EvaluationError(issues.Err(), expression, expressionType, duration)
	...
	if err != nil {
		duration := time.Since(start)
		cel.EvaluationError(err, expression, expressionType, duration)
		metrics.RecordCELEvaluation(expressionType, duration, err)
		return nil, err
	}

	duration := time.Since(start)
	cel.EvaluationSuccess(expression, expressionType, duration, "compiled")
...
func (cc *CompilationCache) Evaluate(ctx context.Context, expression, expressionType string, vars map[string]interface{}) (interface{}, error) {
	...
	value := result.Value()
	cel.EvaluationSuccess(expression, expressionType, duration, value)
...
func GetGlobalCache(env *cel.Env) *CompilationCache {
	globalCacheOnce.Do(func() {
		globalCache = NewCompilationCache(DefaultCacheConfig(), env)
	})
	return globalCache
}

...
const (
	...
	keyEngramDefaultMaxRecursionDepth = "engram.default-max-recursion-depth"
)
...
func (c *ControllerConfig) Clone() ControllerConfig {
	...
	if len(c.StoryRun.Scheduling.Queues) > 0 {
		...
		maps.Copy(copyCfg.StoryRun.Scheduling.Queues, c.StoryRun.Scheduling.Queues)
	}
	return copyCfg
}
...
func NewOperatorConfigManager(k8sClient client.Client, namespace, configMapName string, reader ...client.Reader) (*OperatorConfigManager, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create operator config manager: %w", err)
	}
...
// MustNewOperatorConfigManager constructs an OperatorConfigManager and panics on
// construction failure. Prefer NewOperatorConfigManager in production code so
// startup failures flow through normal error handling; this helper exists for
// tests and one-off callers that treat constructor failure as fatal.
func MustNewOperatorConfigManager(k8sClient client.Client, namespace, configMapName string, reader ...client.Reader) *OperatorConfigManager {
	manager, err := NewOperatorConfigManager(k8sClient, namespace, configMapName, reader...)
	if err != nil {
		panic(err)
	}
	return manager
...
func parseRetryAndTimeouts(cm *corev1.ConfigMap, config *OperatorConfig) {
	...
	configParserLog.Info("Applied retry/timeouts overrides",
		"retry.max-retries", config.Controller.MaxRetries,
		"timeout.default-step", config.Controller.DefaultStepTimeout,
		"timeout.approval-default", config.Controller.ApprovalDefaultTimeout,
		"timeout.external-data-default", config.Controller.ExternalDataTimeout,
		"timeout.conditional-default", config.Controller.ConditionalTimeout)
}
...
//   - Reads "realtime.ttl-seconds-after-finished" for RealtimeTTLSecondsAfterFinished.
...
func parseJobConfig(cm *corev1.ConfigMap, config *OperatorConfig) {
	...
	parseInt32Field(cm, contracts.KeyRealtimeTTLSecondsAfterFinished, &config.Controller.RealtimeTTLSecondsAfterFinished, false)
	...
	configParserLog.Info("Applied job overrides",
		...
		"realtimeTTLSecondsAfterFinished", config.Controller.RealtimeTTLSecondsAfterFinished,
...
//   - Reads "engram.default-max-recursion-depth" and stores in DefaultMaxRecursionDepth.
//   - Propagates the effective recursion depth into the process environment when unset
//     so controller-side storage hydration uses the same global default as workloads.
//
// Arguments:
...
//   - Mutates config.Controller.Engram.EngramControllerConfig.DefaultMaxRecursionDepth.
//
// Notes:
...
func parseEngramDefaults(cm *corev1.ConfigMap, config *OperatorConfig) {
	...
	if parseIntField(cm, keyEngramDefaultMaxRecursionDepth, &config.Controller.Engram.EngramControllerConfig.DefaultMaxRecursionDepth, true) {
		configParserLog.Info("Applied override", "key", keyEngramDefaultMaxRecursionDepth, "value", config.Controller.Engram.EngramControllerConfig.DefaultMaxRecursionDepth)
	}
	setEnvIfEmpty(contracts.MaxRecursionDepthEnv, strconv.Itoa(EffectiveMaxRecursionDepth(config)))
}
...
// setEnvIfEmpty sets an environment variable only if it is not already set.
// Storage env vars are write-once per process lifetime. ConfigMap changes to
// storage configuration require an operator pod restart to take effect.
func setEnvIfEmpty(key, value string) {
	...
	if existing := os.Getenv(key); existing != "" {
		if existing != value {
			configParserLog.Info("storage env var already set; ConfigMap update requires pod restart",
				"key", key, "currentValue", existing, "ignoredValue", value)
		}
		return
	}

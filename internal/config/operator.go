...
//   - Sets apiReader to nil (can be configured later via SetAPIReader).
//
// Arguments:
...
func logConfigSummary(logger interface{ Info(string, ...interface{}) }, config *OperatorConfig, reason ConfigReloadReason) {
	c := &config.Controller
	logger.Info("Operator configuration applied",
		...
		"maxLoopBatchSize", c.MaxLoopBatchSize,
		"maxLoopConcurrency", c.MaxLoopConcurrency,
...
// controller.reconcile-timeout parses as a positive duration.
//
// Behavior:
...
func setReconcileTimeout(cm *corev1.ConfigMap, config *OperatorConfig) {
	var parsed time.Duration
	if parseDurationField(cm, contracts.KeyReconcileTimeout, &parsed, true) {
...
// parseLoopConfig groups the loop.* ConfigMap overrides so the operator can
// cap iterations, batch sizes, and concurrency for loop steps.
//
// Behavior:
//   - Calls setMaxLoopIterations to cap maximum loop iterations.
//   - Calls setDefaultLoopBatchSize to set default batch size.
//   - Calls setMaxLoopBatchSize to cap maximum batch size.
//   - Calls setMaxLoopConcurrency to limit concurrent loop execution.
//   - Calls setMaxConcurrencyLimit to set hard ceiling for concurrency.
//   - Logs a summary of applied loop overrides.
//
// Arguments:
//   - cm *corev1.ConfigMap: the operator ConfigMap containing loop keys.
//   - config *OperatorConfig: mutated in place with parsed loop values.
//
// Side Effects:
//   - Mutates config.Controller loop-related fields via sub-setters.
//   - Logs applied overrides via configParserLog.
func parseLoopConfig(cm *corev1.ConfigMap, config *OperatorConfig) {
	...
	configParserLog.Info("Applied loop overrides",
		"maxIterations", config.Controller.MaxLoopIterations,
		"defaultBatchSize", config.Controller.DefaultLoopBatchSize,
		"maxBatchSize", config.Controller.MaxLoopBatchSize,
		"maxConcurrency", config.Controller.MaxLoopConcurrency,
		"maxConcurrencyLimit", config.Controller.MaxConcurrencyLimit,
	)
}

// setMaxLoopIterations overrides Controller.MaxLoopIterations when
// loop.max-iterations parses as a positive integer.
//
// Behavior:
//   - Reads "loop.max-iterations" from cm.Data.
//   - Parses the value as an integer; ignores non-positive values.
//   - Stores the parsed value in config.Controller.MaxLoopIterations.
//
// Arguments:
//   - cm *corev1.ConfigMap: the operator ConfigMap containing the key.
//   - config *OperatorConfig: mutated in place with the parsed value.
//
// Side Effects:
//   - Mutates config.Controller.MaxLoopIterations if key exists and is valid.
func setMaxLoopIterations(cm *corev1.ConfigMap, config *OperatorConfig) {
	parseIntField(cm, contracts.KeyMaxLoopIterations, &config.Controller.MaxLoopIterations, true)
}

// setDefaultLoopBatchSize overrides Controller.DefaultLoopBatchSize when
// loop.default-batch-size parses as a positive integer.
//
// Behavior:
//   - Reads "loop.default-batch-size" from cm.Data.
//   - Parses the value as an integer; ignores non-positive values.
//   - Stores the parsed value in config.Controller.DefaultLoopBatchSize.
//
// Arguments:
//   - cm *corev1.ConfigMap: the operator ConfigMap containing the key.
//   - config *OperatorConfig: mutated in place with the parsed value.
//
// Side Effects:
//   - Mutates config.Controller.DefaultLoopBatchSize if key exists and is valid.
func setDefaultLoopBatchSize(cm *corev1.ConfigMap, config *OperatorConfig) {
	parseIntField(cm, contracts.KeyDefaultLoopBatchSize, &config.Controller.DefaultLoopBatchSize, true)
}

// setMaxLoopBatchSize overrides Controller.MaxLoopBatchSize when
// loop.max-batch-size parses as a positive integer.
//
// Behavior:
//   - Reads "loop.max-batch-size" from cm.Data.
//   - Parses the value as an integer; ignores non-positive values.
//   - Stores the parsed value in config.Controller.MaxLoopBatchSize.
//
// Arguments:
//   - cm *corev1.ConfigMap: the operator ConfigMap containing the key.
//   - config *OperatorConfig: mutated in place with the parsed value.
//
// Side Effects:
//   - Mutates config.Controller.MaxLoopBatchSize if key exists and is valid.
func setMaxLoopBatchSize(cm *corev1.ConfigMap, config *OperatorConfig) {
	parseIntField(cm, contracts.KeyMaxLoopBatchSize, &config.Controller.MaxLoopBatchSize, true)
}

// setMaxLoopConcurrency overrides Controller.MaxLoopConcurrency when
// loop.max-concurrency parses as a positive integer.
//
// Behavior:
//   - Reads "loop.max-concurrency" from cm.Data.
//   - Parses the value as an integer; ignores non-positive values.
//   - Stores the parsed value in config.Controller.MaxLoopConcurrency.
//
// Arguments:
//   - cm *corev1.ConfigMap: the operator ConfigMap containing the key.
//   - config *OperatorConfig: mutated in place with the parsed value.
//
// Side Effects:
//   - Mutates config.Controller.MaxLoopConcurrency if key exists and is valid.
func setMaxLoopConcurrency(cm *corev1.ConfigMap, config *OperatorConfig) {
	parseIntField(cm, contracts.KeyMaxLoopConcurrency, &config.Controller.MaxLoopConcurrency, true)
}

// setMaxConcurrencyLimit overrides Controller.MaxConcurrencyLimit when
// loop.max-concurrency-limit parses as a positive integer.
//
// Behavior:
//   - Reads "loop.max-concurrency-limit" from cm.Data.
//   - Parses the value as an integer; ignores non-positive values.
//   - Stores the parsed value in config.Controller.MaxConcurrencyLimit.
//
// Arguments:
//   - cm *corev1.ConfigMap: the operator ConfigMap containing the key.
//   - config *OperatorConfig: mutated in place with the parsed value.
//
// Side Effects:
//   - Mutates config.Controller.MaxConcurrencyLimit if key exists and is valid.
//
// Notes:
//   - This is a hard ceiling that Story-level policies cannot exceed.
func setMaxConcurrencyLimit(cm *corev1.ConfigMap, config *OperatorConfig) {
	parseIntField(cm, contracts.KeyMaxConcurrencyLimit, &config.Controller.MaxConcurrencyLimit, true)
}
...
func parseSecurityConfig(cm *corev1.ConfigMap, config *OperatorConfig) {
	...
	if val, exists := cm.Data[contracts.KeyRunAsUser]; exists {
		if parsed, err := strconv.ParseInt(val, 10, 64); err == nil {
			...
			configParserLog.Info("Ignoring invalid value", "key", contracts.KeyRunAsUser, "value", val, "error", err.Error())
		}
	}
...
// parseCELConfig reads cel.* keys from the ConfigMap and applies them to the
// controller config for CEL expression evaluation settings.
//
// Behavior:
//   - Reads "cel.evaluation-timeout" and stores as duration in CELEvaluationTimeout.
//   - Reads "cel.max-expression-length" and stores as int in CELMaxExpressionLength.
//   - Reads "cel.enable-macros" and stores boolean in CELEnableMacros.
//
// Arguments:
//   - cm *corev1.ConfigMap: the operator ConfigMap containing CEL keys.
//   - config *OperatorConfig: mutated in place with the parsed values.
//
// Side Effects:
//   - Mutates config.Controller CEL-related fields.
//
// Notes:
//   - CELMaxExpressionLength must be positive; non-positive values are ignored.
func parseCELConfig(cm *corev1.ConfigMap, config *OperatorConfig) {
	if val, exists := cm.Data[contracts.KeyCELEvaluationTimeout]; exists {
		if parsed, err := time.ParseDuration(val); err == nil {
			config.Controller.CELEvaluationTimeout = parsed
		} else {
			configParserLog.Info("Ignoring invalid value", "key", contracts.KeyCELEvaluationTimeout, "value", val, "error", err.Error())
		}
	}
	if val, exists := cm.Data[contracts.KeyCELMaxExpressionLength]; exists {
		if parsed, err := strconv.Atoi(val); err == nil && parsed > 0 {
			config.Controller.CELMaxExpressionLength = parsed
		} else if err != nil {
			configParserLog.Info("Ignoring invalid value", "key", contracts.KeyCELMaxExpressionLength, "value", val, "error", err.Error())
		}
	}
	if val, exists := cm.Data[contracts.KeyCELEnableMacros]; exists {
		config.Controller.CELEnableMacros = val == strTrue
	}
	configParserLog.Info("Applied CEL overrides",
		"evaluationTimeout", config.Controller.CELEvaluationTimeout,
		"maxExpressionLength", config.Controller.CELMaxExpressionLength,
		"enableMacros", config.Controller.CELEnableMacros,
	)
}
...
func parseStoryRunConfig(cm *corev1.ConfigMap, config *OperatorConfig) {
	...
	parseIntField(cm, contracts.KeyStoryRunMaxConcurrentReconciles, &config.Controller.StoryRun.MaxConcurrentReconciles, true)
...
func parseStepRunConfig(cm *corev1.ConfigMap, config *OperatorConfig) {
	// Use shared helpers for consistent parsing and logging
	parseIntField(cm, contracts.KeyStepRunMaxConcurrentReconciles, &config.Controller.StepRun.MaxConcurrentReconciles, true)
...
func parseStoryConfig(cm *corev1.ConfigMap, config *OperatorConfig) {
	// Use shared helpers for consistent parsing and logging
	parseIntField(cm, contracts.KeyStoryMaxConcurrentReconciles, &config.Controller.Story.MaxConcurrentReconciles, true)
...
func parseEngramConfig(cm *corev1.ConfigMap, config *OperatorConfig) {
	// Use shared helpers for consistent parsing and logging
	parseIntField(cm, contracts.KeyEngramMaxConcurrentReconciles, &config.Controller.Engram.MaxConcurrentReconciles, true)
...
func applyControllerRateLimiterOverrides(
	...
	parseIntField(cm, concurrencyKey, concurrencyTarget, true)

...
// syncConfig is the main reconciliation loop for the config manager
func (m *OperatorConfigManager) syncConfig(ctx context.Context) {
	ticker := time.NewTicker(time.Second) // Sync every second
	defer ticker.Stop()
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				log.FromContext(ctx).Error(err, "Failed to load operator configuration")
			} else if newConfig != nil {
				m.mu.Lock()
				m.currentConfig = newConfig
				m.lastSyncTime = time.Now()
				m.mu.Unlock()
				log.FromContext(ctx).Info("Successfully synced operator configuration")
			}
		}
	}
}

// Start begins the background synchronization of the operator configuration
func (m *OperatorConfigManager) Start(ctx context.Context) {
	go m.syncConfig(ctx)
}

// Stop halts the background synchronization
func (m *OperatorConfigManager) Stop() {
	close(m.stopCh)
}

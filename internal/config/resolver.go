...
import (
	...
	"github.com/bubustack/bobrapet/internal/config/chain"
...
func (cr *Resolver) ResolveExecutionConfig(ctx context.Context, step *runsv1alpha1.StepRun, story *v1alpha1.Story, engram *v1alpha1.Engram, template *catalogv1alpha1.EngramTemplate, storyStep *v1alpha1.Step) (*ResolvedExecutionConfig, error) {
	...
	if operatorConfig == nil {
		resolverLogger.Info("Falling back to default operator config; config manager not ready")
	...
	if step != nil {
		...
		if resolved.ServiceAccountName == "" {
...
func (cr *Resolver) getOperatorDefaults(config *OperatorConfig) *ResolvedExecutionConfig {
	...
	case "file":
		path := strings.TrimSpace(config.Controller.DefaultFileStoragePath)
		claim := strings.TrimSpace(config.Controller.DefaultFileVolumeClaimName)
		if path != "" && claim != "" {
			resolved.Storage = &v1alpha1.StoragePolicy{
				File: &v1alpha1.FileStorageProvider{
					Path:            path,
...
func (cr *Resolver) applyTemplateSecurity(exec *catalogv1alpha1.TemplateExecutionPolicy, config *ResolvedExecutionConfig) error {
	...
	if exec.Security.RecommendedRunAsUser != nil {
		if *exec.Security.RecommendedRunAsUser <= 0 {
			return fmt.Errorf("template recommendedRunAsUser must be greater than 0")
		}
...
// MergeTemplateRetryIntoPolicy fills Delay, MaxDelay, and Backoff on policy from template recommendations
// when the policy's fields are nil. Used when creating a StepRun so the controller uses template retry
// defaults without resolving full config at retry time.
// Returns a new policy (or policy copy) with merged values; does not mutate inputs.
//
//nolint:gocyclo // complex by design
func MergeTemplateRetryIntoPolicy(policy *v1alpha1.RetryPolicy, templateRetry *catalogv1alpha1.TemplateRetryPolicy) *v1alpha1.RetryPolicy {
	if templateRetry == nil {
		return policy
	}
	hasDelay := templateRetry.RecommendedBaseDelay != nil && *templateRetry.RecommendedBaseDelay != ""
	hasMaxDelay := templateRetry.RecommendedMaxDelay != nil && *templateRetry.RecommendedMaxDelay != ""
	hasBackoff := templateRetry.RecommendedBackoff != nil && *templateRetry.RecommendedBackoff != ""
	if !hasDelay && !hasMaxDelay && !hasBackoff {
		return policy
	}
	...
	if policy != nil {
		...
		out.Jitter = policy.Jitter
	}
	if (out.Delay == nil || *out.Delay == "") && hasDelay {
		out.Delay = templateRetry.RecommendedBaseDelay
	}
	if (out.MaxDelay == nil || *out.MaxDelay == "") && hasMaxDelay {
		out.MaxDelay = templateRetry.RecommendedMaxDelay
	}
	if out.Backoff == nil && hasBackoff {
		if b := parseBackoffStrategy(*templateRetry.RecommendedBackoff); b != nil {
			out.Backoff = b
		}
	}
...
// parseBackoffStrategy maps a string (e.g. "exponential", "linear", "constant") to BackoffStrategy.
...
func cloneCachePolicy(in *v1alpha1.CachePolicy) *v1alpha1.CachePolicy {
	if in == nil {
		return nil
	}
	out := &v1alpha1.CachePolicy{}
	if in.Enabled != nil {
		val := *in.Enabled
		out.Enabled = &val
	}
...
func (cr *Resolver) applyEngramConfig(ctx context.Context, engram *v1alpha1.Engram, config *ResolvedExecutionConfig) error {
	...
	if engram.Spec.Secrets != nil {
		...
		maps.Copy(config.Secrets, engram.Spec.Secrets)
	}
	...
	executor.
		...
		Add(chain.Stage{
			...
			Mode: chain.ModeLogAndSkip,
			Fn: func(context.Context) error {
				return applyEngramPlacementOverrides(engram, config)
			},
		}).
		Add(chain.Stage{
			Name: "cache",
			Mode: chain.ModeLogAndSkip,
			Fn: func(context.Context) error {
				return applyEngramCacheOverrides(engram, config)
			},
		}).
		Add(chain.Stage{
...
func applyEngramPlacementOverrides(engram *v1alpha1.Engram, config *ResolvedExecutionConfig) error {
	if engram.Spec.ExecutionPolicy == nil || engram.Spec.ExecutionPolicy.Placement == nil {
		return nil
	}
	applyPlacementPolicy(engram.Spec.ExecutionPolicy.Placement, config)
	return nil
}

func applyEngramCacheOverrides(engram *v1alpha1.Engram, config *ResolvedExecutionConfig) error {
	if engram.Spec.ExecutionPolicy == nil || engram.Spec.ExecutionPolicy.Cache == nil {
		return nil
	}
	config.Cache = cloneCachePolicy(engram.Spec.ExecutionPolicy.Cache)
	return nil
}
...
func (cr *Resolver) ApplyExecutionOverrides(ctx context.Context, overrides *v1alpha1.ExecutionOverrides, config *ResolvedExecutionConfig) error {
	...
	executor.
		Add(chain.Stage{
			...
			Mode: chain.ModeLogAndSkip,
			Fn: func(context.Context) error {
				return applyBasicOverrides(layer, overrides, config)
			},
		}).
		Add(chain.Stage{
			Name: "security",
		...
		Add(chain.Stage{
			...
			Mode: chain.ModeLogAndSkip,
			Fn: func(context.Context) error {
				return applyPlacementOverrides(overrides, config)
			},
		}).
		Add(chain.Stage{
			Name: "cache",
			Mode: chain.ModeLogAndSkip,
			Fn: func(context.Context) error {
				return applyCacheOverrides(overrides, config)
			},
		}).
		Add(chain.Stage{
			Name: "imagePolicy",
...
//
// Behavior:
//   - Ignores overrides.ServiceAccountName to prevent user-authored workload identity escalation.
//   - Ignores overrides.AutomountServiceAccountToken to prevent user-authored token mount escalation.
//   - Applies overrides.Debug to config.DebugLogs.
//
// Arguments:
//   - layer string: override source layer for audit logs.
...
//
// Side Effects:
//   - Mutates config.DebugLogs and MaxInlineSize.
func applyBasicOverrides(layer string, overrides *v1alpha1.ExecutionOverrides, config *ResolvedExecutionConfig) error {
	var applied []string
	if overrides.ServiceAccountName != nil {
		resolverLogger.Info("Ignoring untrusted serviceAccountName override", "layer", layer)
	}
	if overrides.AutomountServiceAccountToken != nil {
		resolverLogger.Info("Ignoring untrusted automountServiceAccountToken override", "layer", layer)
	}
...
func applySecurityOverrides(overrides *v1alpha1.ExecutionOverrides, config *ResolvedExecutionConfig) error {
	...
	if sec.RunAsUser != nil {
		if *sec.RunAsUser <= 0 {
			return fmt.Errorf("execution override runAsUser must be greater than 0")
		}
...
// applyPlacementOverrides applies pod placement constraints from ExecutionOverrides.
//
// Behavior:
//   - Returns early if overrides.Placement is nil.
//   - Merges NodeSelector into config.NodeSelector (overwrites matching keys).
//   - Replaces config.Tolerations when overrides provide a value (including empty list).
//   - Replaces config.Affinity when overrides provide a value.
func applyPlacementOverrides(overrides *v1alpha1.ExecutionOverrides, config *ResolvedExecutionConfig) error {
	if overrides.Placement == nil {
		return nil
	}
	applyPlacementPolicy(overrides.Placement, config)
	return nil
}

func applyCacheOverrides(overrides *v1alpha1.ExecutionOverrides, config *ResolvedExecutionConfig) error {
	if overrides.Cache == nil {
		return nil
	}
	config.Cache = cloneCachePolicy(overrides.Cache)
	return nil
}
...
func (cr *Resolver) applyStoryConfig(ctx context.Context, story *v1alpha1.Story, stepRun *runsv1alpha1.StepRun, storyStep *v1alpha1.Step, config *ResolvedExecutionConfig) error {
	...
	if story.Spec.Policy != nil && story.Spec.Policy.Execution != nil && story.Spec.Policy.Execution.Security != nil {
		if ru := story.Spec.Policy.Execution.Security.RunAsUser; ru != nil && *ru <= 0 {
			return fmt.Errorf("story execution policy runAsUser must be greater than 0")
		}
	}
	if currentStep != nil && currentStep.Execution != nil && currentStep.Execution.Security != nil {
		if ru := currentStep.Execution.Security.RunAsUser; ru != nil && *ru <= 0 {
			return fmt.Errorf("story step execution runAsUser must be greater than 0")
		}
	}
	...
	if currentStep != nil && currentStep.Secrets != nil {
		...
		// Step secrets have higher priority
		maps.Copy(config.Secrets, currentStep.Secrets)
	}
...
//   - Ignores ServiceAccountName to prevent user-authored workload identity escalation.
...
func applyExecutionPolicyDefaults(policy *v1alpha1.ExecutionPolicy, config *ResolvedExecutionConfig) error {
	...
	var errs []error //nolint:prealloc // size not predictable
	errs = append(errs, applyPolicyResourceDefaults(policy.Resources, config)...)
	errs = append(errs, applyPolicySecurityDefaults(policy.Security, config)...)
	applyPolicyPlacementDefaults(policy.Placement, config)
	...
	if policy.ServiceAccountName != nil && strings.TrimSpace(*policy.ServiceAccountName) != "" {
		resolverLogger.Info("Ignoring untrusted story execution serviceAccountName override")
	}
	applyPolicyStorageDefaults(policy.Storage, config)
	applyPolicyCacheDefaults(policy.Cache, config)
	return errors.Join(errs...)
}
...
func applyPolicySecurityDefaults(sec *v1alpha1.SecurityPolicy, config *ResolvedExecutionConfig) []error {
	if sec == nil {
		return nil
	}
	var errs []error
	...
	if sec.RunAsUser != nil {
		if *sec.RunAsUser <= 0 {
			errs = append(errs, fmt.Errorf("execution policy runAsUser must be greater than 0"))
		} else {
			config.RunAsUser = *sec.RunAsUser
		}
	}
...
func applyPolicyPlacementDefaults(placement *v1alpha1.PlacementPolicy, config *ResolvedExecutionConfig) {
	if placement == nil {
		return
	}
	applyPlacementPolicy(placement, config)
}

func applyPlacementPolicy(placement *v1alpha1.PlacementPolicy, config *ResolvedExecutionConfig) {
	if placement == nil {
		return
	}
	if placement.NodeSelector != nil {
		if config.NodeSelector == nil {
			config.NodeSelector = make(map[string]string, len(placement.NodeSelector))
		}
		maps.Copy(config.NodeSelector, placement.NodeSelector)
	}
	if placement.Tolerations != nil {
		config.Tolerations = append([]corev1.Toleration(nil), placement.Tolerations...)
	}
	if placement.Affinity != nil {
		config.Affinity = placement.Affinity.DeepCopy()
	}
	resolverLogger.V(1).Info("Applied placement policy",
		"nodeSelectorKeys", len(placement.NodeSelector),
		"tolerations", len(placement.Tolerations),
		"affinity", placement.Affinity != nil)
}
...
func applyPolicyCacheDefaults(cache *v1alpha1.CachePolicy, config *ResolvedExecutionConfig) {
	if cache == nil {
		return
	}
	config.Cache = cloneCachePolicy(cache)
}
...
//
// Side Effects:
//   - Mutates config.Resources and DebugLogs.
//
// Notes:
...
func applyStepRunExecutionOverrides(stepRun *runsv1alpha1.StepRun, config *ResolvedExecutionConfig) error {
	...
	if overrides.BackoffLimit != nil {
		config.BackoffLimit = *overrides.BackoffLimit
	}
	if overrides.TTLSecondsAfterFinished != nil {
		config.TTLSecondsAfterFinished = *overrides.TTLSecondsAfterFinished
	}
	if overrides.RestartPolicy != nil {
		switch *overrides.RestartPolicy {
		case string(corev1.RestartPolicyNever), string(corev1.RestartPolicyOnFailure):
			config.RestartPolicy = corev1.RestartPolicy(*overrides.RestartPolicy)
		default:
			errs = append(errs, fmt.Errorf("invalid restart policy %q (valid: Never, OnFailure)", *overrides.RestartPolicy))
		}
	}
	resolverLogger.V(1).Info("Applied StepRun resource overrides",
		...
		"backoffLimit", overrides.BackoffLimit, "ttlSecondsAfterFinished", overrides.TTLSecondsAfterFinished, "restartPolicy", overrides.RestartPolicy)
	return errors.Join(errs...)
}
...
func applyStepRunTimeout(stepRun *runsv1alpha1.StepRun, config *ResolvedExecutionConfig) error {
	...
	timeout := strings.TrimSpace(stepRun.Spec.Timeout)
	if timeout == "" {
		return nil
	}
	d, err := time.ParseDuration(timeout)
	if err != nil {
		return fmt.Errorf("stepRun timeout %q invalid: %w", timeout, err)
	}
	config.DefaultStepTimeout = d
	resolverLogger.V(1).Info("Applied StepRun timeout override", "timeout", d.String())
	return nil
}

func applyStepRunRetry(stepRun *runsv1alpha1.StepRun, config *ResolvedExecutionConfig) error {
	if stepRun == nil || stepRun.Spec.Retry == nil || stepRun.Spec.Retry.MaxRetries == nil {
		return nil
	}
	config.MaxRetries = int(*stepRun.Spec.Retry.MaxRetries)
	resolverLogger.V(1).Info("Applied StepRun retry override", "maxRetries", config.MaxRetries)
	return nil
}
...
func (config *ResolvedExecutionConfig) ToResourceRequirements() corev1.ResourceRequirements {
	return corev1.ResourceRequirements{
		Requests: cloneResourceList(config.Resources.Requests),
		Limits:   cloneResourceList(config.Resources.Limits),
	}
}

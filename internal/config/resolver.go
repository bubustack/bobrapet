...
// applyNamespaceConfig applies namespace-level execution policy
func (cr *Resolver) applyNamespaceConfig(ctx context.Context, namespace string, config *ResolvedExecutionConfig) {
	// In this version, we assume namespace-level config is not yet implemented.
...
func (cr *Resolver) applyEngramConfig(engram *v1alpha1.Engram, config *ResolvedExecutionConfig) {
	...
	// Execution Overrides
	if engram.Spec.Overrides != nil {
		exec := engram.Spec.Overrides
		if exec.ServiceAccountName != nil {
			config.ServiceAccountName = *exec.ServiceAccountName
		}
		if exec.Security != nil {
			sec := exec.Security
		...
		if exec.ImagePullPolicy != nil {
			switch *exec.ImagePullPolicy {
			case "Always":
				config.ImagePullPolicy = corev1.PullAlways
			case "Never":
				config.ImagePullPolicy = corev1.PullNever
		...
		if exec.Retry != nil && exec.Retry.MaxRetries != nil {
			config.MaxRetries = int(*exec.Retry.MaxRetries)
		}
	}
}
...
// ToSecurityContext converts to Kubernetes SecurityContext
func (config *ResolvedExecutionConfig) ToSecurityContext() *corev1.SecurityContext {

...
func TestResolveExecutionConfigDefaultsToIdentityHelper(t *testing.T) {
	...
	expected := identity.StoryRunEngramRunnerServiceAccount(storyRunName)
	if cfg.ServiceAccountName != expected {
		t.Fatalf("expected default service account %s, got %s", expected, cfg.ServiceAccountName)
	}
}
...
func (s *recordingSink) record(level int, msg string, kv []interface{}, err error) {
	entryKV := append([]interface{}{}, s.values...)
...
func findValue(kv []interface{}, key string) string {

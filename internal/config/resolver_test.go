...
func TestApplyStoryExecutionPolicyDefaults(t *testing.T) {
	resolved := &ResolvedExecutionConfig{
		ServiceAccountName: "operator-default-sa",
	...
	assertEqual(t, resolved.ServiceAccountName, "operator-default-sa", "ServiceAccountName")
...
func TestApplyStoryConfigStepOverrides(t *testing.T) {
	...
	if resolved.ServiceAccountName != "default" {
		t.Fatalf("expected untrusted service account overrides to be ignored, got %s", resolved.ServiceAccountName)
	}
...
func TestApplyExecutionOverridesIgnoresUntrustedWorkloadIdentityFields(t *testing.T) {
	resolved := &ResolvedExecutionConfig{
		ServiceAccountName:           "operator-default-sa",
		AutomountServiceAccountToken: false,
	}
	serviceAccount := "step-sa"
	automount := true
	debug := true
	maxInline := 4096
	...
	resolver := &Resolver{}
	ctx := WithExecutionOverrideLayer(context.Background(), overrideLayerStoryStep)
	if err := resolver.ApplyExecutionOverrides(ctx, overrides, resolved); err != nil {
		t.Fatalf("ApplyExecutionOverrides returned error: %v", err)
	}

	if resolved.ServiceAccountName != "operator-default-sa" {
		t.Fatalf("expected service account override to be ignored, got %s", resolved.ServiceAccountName)
	}
	if resolved.AutomountServiceAccountToken {
		t.Fatalf("expected automount override to be ignored, got %v", resolved.AutomountServiceAccountToken)
	}
	if !resolved.DebugLogs {
		t.Fatalf("expected debug override to still apply")
	}
	if resolved.MaxInlineSize != maxInline {
		t.Fatalf("expected max inline size override to apply, got %d", resolved.MaxInlineSize)
	}
}
...
func TestResolveExecutionConfigDefaultsToIdentityHelper(t *testing.T) {
	...
	manager, err := NewOperatorConfigManager(fake.NewClientBuilder().WithScheme(scheme).Build(), "default", "cfg")
	if err != nil {
	...
	if cfg.AutomountServiceAccountToken {
		t.Fatalf("expected managed runner service account to preserve operator default automount=false")
	}
}
...
func TestResolveExecutionConfigPrecedenceOrder(t *testing.T) {
	...
	expectedRunnerSA := runsidentity.NewEngramRunner(stepRun.Spec.StoryRunRef.Name).ServiceAccountName()
	if cfg.ServiceAccountName != expectedRunnerSA {
		t.Fatalf("expected managed runner ServiceAccount %s, got %s", expectedRunnerSA, cfg.ServiceAccountName)
	}
	if cfg.AutomountServiceAccountToken {
		t.Fatalf("expected automountServiceAccountToken override to remain disabled by default")
	}
	if !cfg.DebugLogs {

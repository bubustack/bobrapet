...
func (r *ImpulseTemplateReconciler) setupControllerWithOptions(mgr ctrl.Manager, options controller.Options) error {
	...
	// Index ImpulseTemplates by description for efficient lookups
	if err := mgr.GetFieldIndexer().IndexField(ctx, &catalogv1alpha1.ImpulseTemplate{}, "spec.description",
	...
	if err := mgr.GetFieldIndexer().IndexField(ctx, &catalogv1alpha1.ImpulseTemplate{}, "spec.supportedModes",
		func(obj client.Object) []string {
			...
			if len(template.Spec.SupportedModes) > 0 {
				modes := make([]string, len(template.Spec.SupportedModes))
				for i, mode := range template.Spec.SupportedModes {
					modes[i] = string(mode)
				}
				return modes
			}

...
import (
	...
	"github.com/bubustack/bobrapet/pkg/annotations"
...
func (r *EngramReconciler) countEngramTriggersBounded(ctx context.Context, engram *bubushv1alpha1.Engram, maxMarks int) (total int64, pending bool, err error) {
	...
	for i := range stepRuns.Items {
		...
		added, annErr := annotations.EnsureStepRunTriggerTokens(ctx, r.Client, types.NamespacedName{Namespace: run.Namespace, Name: run.Name}, contracts.StepRunTriggerTokenEngram)
		if annErr != nil {

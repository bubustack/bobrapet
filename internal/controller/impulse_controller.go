...
const impulseStatusRefreshInterval = 10 * time.Second
...
func (r *ImpulseReconciler) ensureImpulseWorkloads(
	...
	if managedSA {
		if err := r.ensureImpulseRBAC(ctx, ictx.impulse, ictx.config.ServiceAccountName); err != nil {
...
func (r *ImpulseReconciler) reconcileImpulseWorkload(ctx context.Context, ictx *impulseContext) (ctrl.Result, error) {
	...
	case enums.WorkloadModeStatefulSet:
		return r.ensureWorkload(ctx, ictx, "StatefulSet", func() (*int32, *int32, error) {
			statefulSet, err := r.reconcileStatefulSetWorkload(ctx, ictx.impulse, ictx.template, ictx.config, ictx.serviceName)
			if err != nil {
				return nil, nil, err
			}
	...
	default:
		return r.ensureWorkload(ctx, ictx, "Deployment", func() (*int32, *int32, error) {
			deployment, err := r.reconcileDeployment(ctx, ictx.impulse, ictx.template, ictx.config)
			if err != nil {
				return nil, nil, err
			}
...
func (r *ImpulseReconciler) reconcileDeployment(
	...
	return workload.Ensure(ctx, workload.EnsureOptions[*appsv1.Deployment]{
		...
		BuildDesired: func() (*appsv1.Deployment, error) {
			return r.buildDeploymentForImpulse(impulse, engramTemplate, execCfg), nil
		},
...
func (r *ImpulseReconciler) reconcileStatefulSetWorkload(
	...
	return workload.Ensure(ctx, workload.EnsureOptions[*appsv1.StatefulSet]{
		...
		BuildDesired: func() (*appsv1.StatefulSet, error) {
			return r.buildStatefulSetForImpulse(impulse, engramTemplate, execCfg, serviceName), nil
		},
...
func (r *ImpulseReconciler) ensureImpulseServiceAccount(
	...
	managed := resolved.ServiceAccountName == "" || resolved.ServiceAccountName == config.DefaultServiceAccountName
	if !managed {
		kubeutil.RecordEvent(
			r.Recorder,
			log.V(0),
...
func (r *ImpulseReconciler) ensureImpulseRBAC(ctx context.Context, impulse *v1alpha1.Impulse, saName string) error {
	if saName == "" {
...
func (r *ImpulseReconciler) listStoryRunsByImpulse(ctx context.Context, impulse *v1alpha1.Impulse) ([]runsv1alpha1.StoryRun, error) {
	...
	if err := r.List(ctx, &runs, nsOpt, client.MatchingFields{contracts.IndexStoryRunImpulseRef: impulse.Name}); err == nil {
...
func (r *ImpulseReconciler) buildWorkloadBase(
	...
	identity := newImpulseIdentity(impulse)
	replicas := int32(1)
	podSpec := r.buildImpulsePodTemplate(impulse, execCfg, identity.selectorLabels, engramTemplate)
...
func (r *ImpulseReconciler) buildDeploymentForImpulse(impulse *v1alpha1.Impulse, engramTemplate *catalogv1alpha1.EngramTemplate, execCfg *config.ResolvedExecutionConfig) *appsv1.Deployment {
	base := r.buildWorkloadBase(impulse, engramTemplate, execCfg)
...
func (r *ImpulseReconciler) buildStatefulSetForImpulse(impulse *v1alpha1.Impulse, engramTemplate *catalogv1alpha1.EngramTemplate, execCfg *config.ResolvedExecutionConfig, serviceName string) *appsv1.StatefulSet {
	base := r.buildWorkloadBase(impulse, engramTemplate, execCfg)
...
//
// Behavior:
//   - Uses enqueueImpulsesByField with spec.storyRef.name index.
//   - Applies a namespace filter to match only Impulses targeting the Story's namespace.
//
// Arguments:
//   - ctx context.Context: propagated to field listing.
...
//   - fieldKey string: the field index key (e.g., "spec.templateRef.name").
...
//
// Behavior:
//   - Supports "spec.templateRef.name" and "spec.storyRef.name" field keys.

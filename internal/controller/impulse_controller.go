...
// +kubebuilder:rbac:groups=rbac.authorization.k8s.io,resources=roles,verbs=get;list;watch;create;patch;bind;escalate
...
func (r *ImpulseReconciler) prepareImpulseContext(
	...
	story, stop, err := r.loadTargetStory(ctx, impulse, log)
	if err != nil {
		return nil, nil, err
	} else if stop {
		return nil, &ctrl.Result{}, nil
...
func validateImpulseServiceAccountOwnership(sa *corev1.ServiceAccount, impulse *v1alpha1.Impulse) error {
	if sa == nil || impulse == nil || !impulseServiceAccountExists(sa) {
		return nil
	}
	for _, ownerRef := range sa.OwnerReferences {
		if ownerRef.APIVersion != v1alpha1.GroupVersion.String() || ownerRef.Kind != "Impulse" || ownerRef.Name != impulse.Name {
			continue
		}
		if impulse.UID != "" && ownerRef.UID != impulse.UID {
			continue
		}
	...
	return fmt.Errorf(
		"refusing to adopt existing ServiceAccount %s/%s: owner reference does not match Impulse %s",
		sa.Namespace,
		sa.Name,
		impulse.Name,
	)
...
func impulseServiceAccountExists(sa *corev1.ServiceAccount) bool {
	if sa == nil {
		return false
	}
	return sa.GetResourceVersion() != "" || sa.GetUID() != "" || !sa.GetCreationTimestamp().Time.IsZero()
}
...
func (r *ImpulseReconciler) reconcileImpulseWorkload(ctx context.Context, ictx *impulseContext) (ctrl.Result, error) {
	...
	case enums.WorkloadModeStatefulSet:
		return r.ensureWorkload(ctx, ictx, "StatefulSet", func() (*int32, *int32, error) {
			statefulSet, err := r.reconcileStatefulSetWorkload(ctx, ictx.impulse, ictx.story, ictx.template, ictx.config, ictx.deliveryPolicy, ictx.serviceName)
			if err != nil {
				return nil, nil, err
			}
	...
	default:
		return r.ensureWorkload(ctx, ictx, "Deployment", func() (*int32, *int32, error) {
			deployment, err := r.reconcileDeployment(ctx, ictx.impulse, ictx.story, ictx.template, ictx.config, ictx.deliveryPolicy)
			if err != nil {
				return nil, nil, err
			}
...
func (r *ImpulseReconciler) reconcileDeployment(
	...
	return workload.Ensure(ctx, workload.EnsureOptions[*appsv1.Deployment]{
		...
		BuildDesired: func() (*appsv1.Deployment, error) {
			return r.buildDeploymentForImpulse(impulse, story, engramTemplate, execCfg, deliveryPolicy), nil
		},
...
func (r *ImpulseReconciler) reconcileStatefulSetWorkload(
	...
	return workload.Ensure(ctx, workload.EnsureOptions[*appsv1.StatefulSet]{
		...
		BuildDesired: func() (*appsv1.StatefulSet, error) {
			return r.buildStatefulSetForImpulse(impulse, story, engramTemplate, execCfg, deliveryPolicy, serviceName), nil
		},
...
func (r *ImpulseReconciler) loadTargetStory(ctx context.Context, impulse *v1alpha1.Impulse, log *logging.ControllerLogger) (*v1alpha1.Story, bool, error) {
	...
	if err := webhookshared.ValidateCrossNamespaceReference(
		...
		if r.Recorder != nil {
			r.Recorder.Eventf(impulse, nil, corev1.EventTypeWarning, conditions.ReasonStoryReferenceInvalid, "Reconcile", message)
		}
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			if r.Recorder != nil {
				r.Recorder.Eventf(impulse, nil, corev1.EventTypeWarning, conditions.ReasonStoryNotFound, "Reconcile", message)
			}
...
func (r *ImpulseReconciler) resolveImpulseConfig(
	...
	if resolveErr != nil {
		if r.Recorder != nil {
			r.Recorder.Eventf(impulse, nil, corev1.EventTypeWarning, conditions.ReasonInvalidConfiguration, "Reconcile",
	...
	if impulse.Spec.Execution != nil && impulse.Spec.Execution.ServiceAccountName != nil {
		if saName := strings.TrimSpace(*impulse.Spec.Execution.ServiceAccountName); saName != "" {
...
func (r *ImpulseReconciler) ensureImpulseServiceAccount(
	...
	if !managed {
		// Impulse workloads still need an in-cluster token to watch resources and
		// submit StoryTriggers, even when the ServiceAccount itself is user-managed.
	...
	if _, err := controllerutil.CreateOrPatch(ctx, r.Client, sa, func() error {
		if impulseServiceAccountExists(sa) {
			if err := validateImpulseServiceAccountOwnership(sa, impulse); err != nil {
				return err
			}
		}
...
// ServiceAccount to submit StoryTrigger requests and observe the resulting
// StoryRun on behalf of an Impulse.
//
// Behavior:
	...
	roleRules := []rbacv1.PolicyRule{
		{
			...
			Resources: []string{"storytriggers"},
...
func resolveImpulseRBACRules(template *catalogv1alpha1.ImpulseTemplate, impulse *v1alpha1.Impulse) []rbacv1.PolicyRule {
	var merged []rbacv1.PolicyRule
	if template != nil && template.Spec.Execution != nil && template.Spec.Execution.RBAC != nil {
		merged = mergePolicyRules(merged, template.Spec.Execution.RBAC.Rules)
	}
	if impulse != nil && impulse.Spec.Execution != nil && impulse.Spec.Execution.RBAC != nil {
		merged = mergePolicyRules(merged, impulse.Spec.Execution.RBAC.Rules)
	}
	if len(merged) == 0 {
		return nil
	}
...
func (r *ImpulseReconciler) buildWorkloadBase(
	...
	identity := newImpulseIdentity(impulse)
	replicas := int32(1)
	podSpec := r.buildImpulsePodTemplate(impulse, story, execCfg, identity.selectorLabels, engramTemplate, deliveryPolicy)
...
func (r *ImpulseReconciler) buildDeploymentForImpulse(
	...
	base := r.buildWorkloadBase(impulse, story, engramTemplate, execCfg, deliveryPolicy)
...
func (r *ImpulseReconciler) buildImpulsePodTemplate(
	...
	maxRecursionDepth := config.ResolveStoryMaxRecursionDepth(story, config.EffectiveMaxRecursionDepth(r.ConfigResolver.GetOperatorConfig()))
	envVars := []corev1.EnvVar{
		...
		{Name: contracts.MaxInlineSizeEnv, Value: strconv.Itoa(execCfg.MaxInlineSize)},
...
func (r *ImpulseReconciler) SetupWithManager(mgr ctrl.Manager, opts controller.Options) error {
	r.Recorder = mgr.GetEventRecorder("impulse-controller")

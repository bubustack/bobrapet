...
// +kubebuilder:rbac:groups=catalog.bubu.sh,resources=impulsetemplates,verbs=get;list;watch
...
// the Impulse object against the actual cluster state, and then
...
func (r *ImpulseReconciler) reconcileDeployment(ctx context.Context, impulse *v1alpha1.Impulse, template *catalogv1alpha1.ImpulseTemplate, config *config.ResolvedExecutionConfig) (*appsv1.Deployment, error) {
	...
	if !reflect.DeepEqual(deployment.Spec.Template, desiredDeployment.Spec.Template) {
		...
		if err := r.ControllerDependencies.Client.Update(ctx, deployment); err != nil {
	...
	if impulse.Status.Phase != newPhase || impulse.Status.ReadyReplicas != deployment.Status.ReadyReplicas {
		err := patch.RetryableStatusPatch(ctx, r.ControllerDependencies.Client, impulse, func(obj client.Object) {
			...
			if newPhase == enums.PhaseRunning && message == "Impulse is active and listening for events." {
				cm.SetCondition(&i.Status.Conditions, conditions.ConditionReady, metav1.ConditionTrue, "DeploymentReady", message)
			} else {
				cm.SetCondition(&i.Status.Conditions, conditions.ConditionReady, metav1.ConditionFalse, "DeploymentNotReady", message)
			}
		})
		if err != nil {

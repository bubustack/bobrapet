...
func (r *DAGReconciler) runDagIterations(ctx context.Context, srun *runsv1alpha1.StoryRun, state *dagPipelineState) error {
	...
	for i := 0; i < len(allSteps)+1; i++ {
		...
		var err error
		state.stepRunList, state.priorOutputs, err = r.refreshAfterSubStoriesIfNeeded(ctx, srun, allSteps, state.stepRunList, state.priorOutputs)
		if err != nil {
			return err
		}
...
func (r *DAGReconciler) enforceStoryTimeout(ctx context.Context, srun *runsv1alpha1.StoryRun, story *bubuv1alpha1.Story, log *logging.ControllerLogger) (bool, error) {
	...
	srun.Status.Error = runsstatus.BuildStoryRunError(conditions.ReasonTimedOut, message, nil, srun.Status.StepStates)
	if err := r.setStoryRunPhase(ctx, srun, enums.PhaseTimeout, conditions.ReasonTimedOut, message); err != nil {
		return true, err
	}
	if log != nil {
...
func (r *DAGReconciler) refreshAfterSubStoriesIfNeeded(ctx context.Context, srun *runsv1alpha1.StoryRun, steps []bubuv1alpha1.Step, stepRunList *runsv1alpha1.StepRunList, prior map[string]any) (*runsv1alpha1.StepRunList, map[string]any, error) {
...
func (r *DAGReconciler) finalizeStoryRun(
	...
	case len(finalFailed) > 0:
		...
		srun.Status.Error = runsstatus.BuildStoryRunError(conditions.ReasonCleanupFailed, message, finalFailed, srun.Status.StepStates)
		return r.setStoryRunPhase(ctx, srun, enums.PhaseFailed, conditions.ReasonCleanupFailed, message)
	case len(compFailed) > 0:
		stepName := firstFailedStepName(compFailed)
		message := fmt.Sprintf("Compensation step %s failed.", stepName)
		srun.Status.Error = runsstatus.BuildStoryRunError(conditions.ReasonCompensationFailed, message, compFailed, srun.Status.StepStates)
		return r.setStoryRunPhase(ctx, srun, enums.PhaseFailed, conditions.ReasonCompensationFailed, message)
	case len(mainFailed) > 0:
		if len(story.Spec.Compensations) > 0 {
			message := "Story failed; compensations completed."
			srun.Status.Error = runsstatus.BuildStoryRunError(conditions.ReasonCompensated, message, mainFailed, srun.Status.StepStates)
			return r.setStoryRunPhase(ctx, srun, enums.PhaseCompensated, conditions.ReasonCompensated, message)
		}
		stepName := firstFailedStepName(mainFailed)
		message := fmt.Sprintf("Story completed with failed step: %s.", stepName)
		srun.Status.Error = runsstatus.BuildStoryRunError(conditions.ReasonDependencyFailed, message, mainFailed, srun.Status.StepStates)
		return r.setStoryRunPhase(ctx, srun, enums.PhaseFailed, conditions.ReasonDependencyFailed, message)
	default:
		if err := r.finalizeSuccessfulRun(ctx, srun, story); err != nil {
			...
			srun.Status.Error = runsstatus.BuildStoryRunError(conditions.ReasonOutputResolutionFailed, err.Error(), nil, srun.Status.StepStates)
			_ = r.setStoryRunPhase(ctx, srun, enums.PhaseFailed, conditions.ReasonOutputResolutionFailed, "Failed to evaluate final output template.")
			return err
		}
		return nil
	}
	...
	for i := range steps {
		...
		if step.Type == enums.StepTypeExecuteStory && step.With != nil {
			...
			if err != nil {
				...
				state := runsv1alpha1.StepState{Phase: enums.PhaseFailed, Message: "Failed to get sub-StoryRun"}
				state = ensureStepStateTimes(state, metav1.Now())
				srun.Status.StepStates[step.Name] = state
				statusUpdated = true
				continue
...
func (r *DAGReconciler) checkSyncSleepSteps(ctx context.Context, srun *runsv1alpha1.StoryRun, story *bubuv1alpha1.Story, steps []bubuv1alpha1.Step, state *dagPipelineState) bool {
	...
	for i := range steps {
		...
		if cfg.duration <= 0 {
			next.Phase = enums.PhaseSucceeded
			next.Message = "Sleep completed."
		} else {
			if next.StartedAt == nil {
				next.StartedAt = &now
			}
			sleepUntil := next.StartedAt.Time.Add(cfg.duration)
			if state != nil && state.timers != nil {
			...
			if remaining <= 0 {
				...
				if next.Message == "" || next.Message == "Sleep completed." {
...
func (r *DAGReconciler) checkSyncWaitSteps(ctx context.Context, srun *runsv1alpha1.StoryRun, story *bubuv1alpha1.Story, steps []bubuv1alpha1.Step, state *dagPipelineState) bool {
	...
	inputs, err := getStoryRunInputs(srun, story)
	if err != nil {
	...
	for i := range steps {
		...
		if waiting {
			if cfg.timeout != nil {
				...
				timeoutAt := next.StartedAt.Time.Add(*cfg.timeout)
				if state != nil && state.timers != nil {
...
func (r *DAGReconciler) findAndLaunchReadySteps(ctx context.Context, srun *runsv1alpha1.StoryRun, story *bubuv1alpha1.Story, steps []bubuv1alpha1.Step, completedSteps, runningSteps map[string]bool, priorStepOutputs map[string]any, depPolicy stepDependencyPolicy, needsPersist *bool) ([]*bubuv1alpha1.Step, []*bubuv1alpha1.Step, int, error) {
	...
	storyRunInputs, _ := getStoryRunInputs(srun, story)
...
func (r *DAGReconciler) enforceSchedulingLimits(ctx context.Context, srun *runsv1alpha1.StoryRun, story *bubuv1alpha1.Story, readySteps []*bubuv1alpha1.Step) ([]*bubuv1alpha1.Step, []*bubuv1alpha1.Step, string, error) {
	...
	if globalLimit > 0 {
		...
		runningGlobal = count
		globalSlots = int(globalLimit) - runningGlobal
		if globalSlots < 0 {
			globalSlots = 0
		}
	}
	...
	if queueLimit > 0 {
		...
		runningQueue = count
		queueSlots = int(queueLimit) - runningQueue
		if queueSlots < 0 {
			queueSlots = 0
		}
	}
...
func (r *DAGReconciler) countRunningStepRuns(ctx context.Context, namespace, storyName string) (int, error) {
	...
	if err := r.Client.List(ctx, &stepRuns,
...
func (r *DAGReconciler) countRunningStepRunsGlobal(ctx context.Context) (int, error) {
	var stepRuns runsv1alpha1.StepRunList
	if err := r.Client.List(ctx, &stepRuns); err != nil {
		return 0, err
	}
	count := 0
	for i := range stepRuns.Items {
...
func (r *DAGReconciler) enforcePriorityOrdering(ctx context.Context, srun *runsv1alpha1.StoryRun, queueName string, queueLabel string, priority int32) (bool, string, error) {
	...
	if err := r.Client.List(ctx, &runs, client.MatchingLabels{contracts.QueueLabelKey: queueLabel}); err != nil {
		return false, "", err
	}
	now := time.Now()
...
func hydratePriorOutputs(ctx context.Context, outputs map[string]any, log *logging.ReconcileLogger) {
	...
	for stepID, raw := range outputs {
		...
		hydrated, err := manager.Hydrate(ctx, outputVal)
		if err != nil {
			if log != nil {
...
func (r *DAGReconciler) findReadySteps(
	...
	for i := range steps {
		...
		if allDepsMet {
			if step.If != nil && *step.If != "" && !isStreaming {
				var result bool
				var err error
...
func (r *DAGReconciler) finalizeSuccessfulRun(ctx context.Context, srun *runsv1alpha1.StoryRun, story *bubuv1alpha1.Story) error {
	...
	storyRunInputs, err := getStoryRunInputs(srun, story)
	if err != nil {
...
// getStoryRunInputs resolves StoryRun inputs via the shared runsinputs helper.
//
// Behavior:
...
func getStoryRunInputs(srun *runsv1alpha1.StoryRun, story *bubuv1alpha1.Story) (map[string]any, error) {
	return runsinputs.ResolveStoryRunInputs(story, srun)
}

...
type recordingStepRunListClient struct {
	client.Client
	lastFieldSelectorValue string
}

func (c *recordingStepRunListClient) List(ctx context.Context, list client.ObjectList, opts ...client.ListOption) error {
	var listOptions client.ListOptions
	for _, opt := range opts {
		opt.ApplyToList(&listOptions)
	}
	if listOptions.FieldSelector == nil {
		return fmt.Errorf("missing StepRun phase field selector")
	}
	value, found := listOptions.FieldSelector.RequiresExactMatch(setup.StepRunPhaseField)
	if !found {
		return fmt.Errorf("missing exact match selector for %s", setup.StepRunPhaseField)
	}
	c.lastFieldSelectorValue = value

	stepRuns, ok := list.(*runsv1alpha1.StepRunList)
	if !ok {
		return fmt.Errorf("unexpected list type %T", list)
	}
	stepRuns.Items = []runsv1alpha1.StepRun{
		{Status: runsv1alpha1.StepRunStatus{Phase: enums.PhaseRunning}},
		{Status: runsv1alpha1.StepRunStatus{Phase: enums.PhaseRunning}},
	}
	return nil
}
...
func TestCountRunningStepRunsGlobalUsesRunningPhaseFieldSelector(t *testing.T) {
	t.Parallel()

	recordingClient := &recordingStepRunListClient{}
	reconciler := &DAGReconciler{Client: recordingClient}

	count, err := reconciler.countRunningStepRunsGlobal(context.Background())
	if err != nil {
		t.Fatalf("countRunningStepRunsGlobal returned error: %v", err)
	}
	if recordingClient.lastFieldSelectorValue != string(enums.PhaseRunning) {
		t.Fatalf("expected field selector %s=%s, got %q", setup.StepRunPhaseField, enums.PhaseRunning, recordingClient.lastFieldSelectorValue)
	}
	if count != 2 {
		t.Fatalf("expected 2 running StepRuns, got %d", count)
	}
}
...
func TestFinalizeStoryRunCompensated(t *testing.T) {
	...
	reconciler := &DAGReconciler{Client: cl}
	...
	if err := cl.Get(ctx, types.NamespacedName{Name: srun.Name, Namespace: srun.Namespace}, &updated); err != nil {
...
func TestDAGReconcileFailsOnUnknownDependency(t *testing.T) {
	scheme := runtime.NewScheme()
	if err := runsv1alpha1.AddToScheme(scheme); err != nil {
		t.Fatalf("failed to register scheme: %v", err)
	}
	...
	story := &bubuv1alpha1.Story{
		Spec: bubuv1alpha1.StorySpec{
			Steps: []bubuv1alpha1.Step{
				{Name: "render", Needs: []string{"missing-step"}},
			},
		},
	}
	...
	reconciler := &DAGReconciler{Client: cl}
	if _, err := reconciler.Reconcile(context.Background(), srun, story); err != nil {
		t.Fatalf("unexpected reconcile error: %v", err)
	}

	var updated runsv1alpha1.StoryRun
	if err := cl.Get(context.Background(), types.NamespacedName{Name: srun.Name, Namespace: srun.Namespace}, &updated); err != nil {
		t.Fatalf("failed to fetch storyrun: %v", err)
	}
	if updated.Status.Phase != enums.PhaseFailed {
		t.Fatalf("expected phase %s, got %s", enums.PhaseFailed, updated.Status.Phase)
	}
	if !strings.Contains(updated.Status.Message, "unknown step dependencies") {
		t.Fatalf("expected invalid graph message, got %q", updated.Status.Message)
	}
	if updated.Status.Error == nil || len(updated.Status.Error.Raw) == 0 {
		t.Fatal("expected structured status error to be persisted")
	}
}

func TestDAGReconcilePersistsStepRunPhaseSync(t *testing.T) {
	t.Parallel()

	scheme := runtime.NewScheme()
	if err := runsv1alpha1.AddToScheme(scheme); err != nil {
		t.Fatalf("failed to register runs scheme: %v", err)
	}
	if err := bubuv1alpha1.AddToScheme(scheme); err != nil {
		t.Fatalf("failed to register story scheme: %v", err)
	}
	...
	srun := &runsv1alpha1.StoryRun{
		...
		Status: runsv1alpha1.StoryRunStatus{
			...
			StepStates: map[string]runsv1alpha1.StepState{
				"greet": {Phase: enums.PhasePending},
			},
		},
	}
	...
	story := &bubuv1alpha1.Story{
		...
		Spec: bubuv1alpha1.StorySpec{
			Steps: []bubuv1alpha1.Step{{Name: "greet"}},
		},
	}

	cl := fake.NewClientBuilder().
		WithScheme(scheme).
		WithStatusSubresource(srun, stepRun).
		WithObjects(srun, stepRun).
		Build()
	...
	if got := updated.Status.StepStates["greet"].Phase; got != enums.PhaseRunning {
		t.Fatalf("expected synced step state to persist as %s, got %s", enums.PhaseRunning, got)
	}
}

func TestDAGReconcileFailsOnDependencyCycle(t *testing.T) {
	scheme := runtime.NewScheme()
	if err := runsv1alpha1.AddToScheme(scheme); err != nil {
		t.Fatalf("failed to register scheme: %v", err)
	}
	...
	story := &bubuv1alpha1.Story{
		Spec: bubuv1alpha1.StorySpec{
			Steps: []bubuv1alpha1.Step{
				{Name: "render", Needs: []string{"render"}},
			},
		},
	}
	...
	if !strings.Contains(updated.Status.Message, "dependency cycle detected") {
		t.Fatalf("expected dependency cycle message, got %q", updated.Status.Message)
	}
	if updated.Status.Error == nil || len(updated.Status.Error.Raw) == 0 {
		t.Fatal("expected structured status error to be persisted")
	}
...
func TestFinalizeStoryRunPersistsStructuredError(t *testing.T) {
	scheme := runtime.NewScheme()
	if err := runsv1alpha1.AddToScheme(scheme); err != nil {
		t.Fatalf("failed to register scheme: %v", err)
	}
	srun := &runsv1alpha1.StoryRun{
		...
		Status: runsv1alpha1.StoryRunStatus{
			StepStates: map[string]runsv1alpha1.StepState{
				"main": {Phase: enums.PhaseFailed, Message: "boom"},
			},
		},
	}
	story := &bubuv1alpha1.Story{
	...
	reconciler := &DAGReconciler{Client: cl}
	if err := reconciler.finalizeStoryRun(context.Background(), srun, story, map[string]bool{"main": true}, nil, nil, nil); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if updated.Status.Error == nil || !strings.Contains(string(updated.Status.Error.Raw), conditions.ReasonDependencyFailed) {
		t.Fatalf("expected persisted dependency failure error, got %#v", updated.Status.Error)
	}
}

func TestHydratePriorOutputsWithManagerLeavesKubeRefsOpaque(t *testing.T) {
	tmpDir := t.TempDir()
	t.Setenv(contracts.StorageProviderEnv, contracts.StorageProviderFile)
	t.Setenv(contracts.StoragePathEnv, tmpDir)
	...
	payload, err := json.Marshal(storage.StoredObject{
		ContentType: "json",
		Data: json.RawMessage(`{
			"nested":"value",
			"secret":{"$bubuSecretRef":"kube-system/cluster-creds:token"}
		}`),
	})
	if err != nil {
		t.Fatalf("marshal stored object: %v", err)
	}
	refPath := filepath.Join("storyruns", "test", "outputs", "step.json")
	fullPath := filepath.Join(tmpDir, refPath)
	...
	hydratePriorOutputsWithManager(context.Background(), manager, outputs, nil)

	stepContext, ok := outputs["fetch"].(map[string]any)
	if !ok {
		t.Fatalf("expected fetch step context map, got %T", outputs["fetch"])
	}
	expected := map[string]any{
		"nested": "value",
		"secret": map[string]any{
			"$bubuSecretRef": "kube-system/cluster-creds:token",
		},
	}
	if got := stepContext["output"]; !reflect.DeepEqual(got, expected) {
		t.Fatalf("expected hydrated output to keep kube refs opaque, got %#v", got)
	}
	if got := stepContext["outputs"]; !reflect.DeepEqual(got, expected) {
		t.Fatalf("expected hydrated outputs to keep kube refs opaque, got %#v", got)
	}
}

func TestRefreshAfterSubStoriesIfNeededReportsStatusMutation(t *testing.T) {
	scheme := runtime.NewScheme()
	if err := runsv1alpha1.AddToScheme(scheme); err != nil {
		t.Fatalf("failed to register scheme: %v", err)
	}
	...
	steps := []bubuv1alpha1.Step{
		{
			...
			With: &runtime.RawExtension{Raw: []byte(`{"waitForCompletion":true}`)},
		},
	}

	_, _, updated, err := reconciler.refreshAfterSubStoriesIfNeeded(context.Background(), parent, steps, &runsv1alpha1.StepRunList{}, map[string]any{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !updated {
		t.Fatal("expected sub-story refresh to report status mutation")
	}
	if parent.Status.StepStates["call-child"].Phase != enums.PhaseSucceeded {
		t.Fatalf("expected parent step to be marked succeeded, got %s", parent.Status.StepStates["call-child"].Phase)
	}
}
...
func TestEnforcePriorityOrderingBlocksLowerPriority(t *testing.T) {
	...
	cl := fake.NewClientBuilder().WithScheme(scheme).WithObjects(high, low).Build()
	reconciler := &DAGReconciler{Client: cl}
...
func TestCheckSyncSubStoriesWaitsWhenChildNameMissing(t *testing.T) {
	srun := &runsv1alpha1.StoryRun{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: "default",
		},
	...
	reconciler := &DAGReconciler{}
	updated := reconciler.checkSyncSubStories(context.Background(), srun, steps)
	if updated {
		t.Fatalf("expected no status update when child StoryRun name is missing")
	}
	state := srun.Status.StepStates["call-child"]
	if state.Phase != enums.PhaseRunning {
		t.Fatalf("expected step to stay running, got %s", state.Phase)
	}
	if state.Message != "still running" {
		t.Fatalf("expected existing message to be preserved, got %q", state.Message)
	}
}

func TestCheckSyncSubStoriesRetriesOnTransientGetError(t *testing.T) {
	scheme := runtime.NewScheme()
	if err := runsv1alpha1.AddToScheme(scheme); err != nil {
		t.Fatalf("failed to register run scheme: %v", err)
	}
	baseClient := fake.NewClientBuilder().WithScheme(scheme).Build()
	childKey := types.NamespacedName{Namespace: "default", Name: "child-storyrun"}
	...
	updated := reconciler.checkSyncSubStories(context.Background(), srun, steps)
	if updated {
		t.Fatalf("expected no status update on transient child StoryRun get error")
	}
	state := srun.Status.StepStates["call-child"]
	if state.Phase != enums.PhaseRunning {
		t.Fatalf("expected step to stay running, got %s", state.Phase)
...
func TestDependencySatisfiedForRealtime(t *testing.T) {
	reconciler := &DAGReconciler{}
	story := &bubuv1alpha1.Story{Spec: bubuv1alpha1.StorySpec{Pattern: enums.RealtimePattern}}
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := reconciler.dependencySatisfiedForRealtime(story, state); got != tc.expect {
				t.Fatalf("expected %v for phase %s, got %v", tc.expect, tc.phase, got)
			}
		})
	}

	// Non-realtime stories should never short-circuit dependencies.
	batchStory := &bubuv1alpha1.Story{Spec: bubuv1alpha1.StorySpec{Pattern: enums.BatchPattern}}
	if reconciler.dependencySatisfiedForRealtime(batchStory, runsv1alpha1.StepState{Phase: enums.PhaseRunning}) {
		t.Fatalf("expected non-realtime stories to always require completed dependencies")
	}
}
...
func TestInsertSignalValueRespectsSeq(t *testing.T) {
	...
	if got["value"] != "keep" { //nolint:goconst
...
func TestDAGReconciler_RealtimeTopologyTerminatedTriggersCompensation(t *testing.T) {
	// Realtime story with Running main steps + Degraded(TopologyTerminated) condition
	...
	if !mainDone && story.Spec.Pattern == enums.RealtimePattern {
		if conditions.IsDegraded(srun.Status.Conditions) {
			for _, c := range srun.Status.Conditions {
				if c.Type == conditions.ConditionDegraded &&
					...
					_, _, mainFailed, _ = buildStateMaps(story.Spec.Steps, srun.Status.StepStates)
					break
				}
			}
		}
	...
	for _, name := range []string{"step-a", "step-b"} {
		...
		if state.Message != "realtime topology terminated" {
			t.Fatalf("expected %s message 'realtime topology terminated', got %q", name, state.Message)
		}
	}
...
//nolint:gocyclo // complex by design
func TestDAGReconciler_RealtimeTopologyTerminatedTriggersFinally(t *testing.T) {
	// Realtime story with Running main steps + Degraded(TopologyTerminated) condition
				...
				if c.Type == conditions.ConditionDegraded &&
					...
					for i := range story.Spec.Steps {
						...
						if stepState, ok := srun.Status.StepStates[step.Name]; ok {
							if !stepState.Phase.IsTerminal() {
								stepState.Phase = enums.PhaseFailed
								stepState.Message = "realtime topology terminated"
								srun.Status.StepStates[step.Name] = stepState
								mainFailed[step.Name] = true
							}
						}
...
func TestDAGReconciler_RealtimeNoDegradedSkipsTermination(t *testing.T) {
	// Realtime story with Running main steps but NO Degraded condition → stays in main phase.
	steps := []bubuv1alpha1.Step{
		{Name: "step-a"},
		{Name: "step-b"},
	}

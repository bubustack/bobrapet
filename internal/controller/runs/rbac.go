...
var (
	allowedStoryRBACVerbs = map[string]struct{}{
		"get": {},
	}
	allowedStoryRBACResources = map[string]struct{}{
		"configmaps": {},
		"secrets":    {},
	}
)
...
// Behavior:
//   - Calls controllerutil.CreateOrPatch with a mutation callback that lazily allocates the
...
//
// Returns:
//   - error wrapping CreateOrPatch failures; nil on success.
//
// Side Effects:
...
func (r *RBACManager) reconcileServiceAccount(ctx context.Context, storyRun *runsv1alpha1.StoryRun, story *bubuv1alpha1.Story, saName string, log *logging.ReconcileLogger) error {
	...
	op, err := controllerutil.CreateOrPatch(ctx, r.Client, sa, func() error {
		if existingObjectMeta(sa) {
			if err := validateStoryRunServiceAccountOwnership(sa, storyRun); err != nil {
				return err
			}
		}
	...
	if err != nil {
		return fmt.Errorf("failed to create or patch ServiceAccount: %w", err)
	}
...
func (r *RBACManager) reconcileRole(ctx context.Context, storyRun *runsv1alpha1.StoryRun, saName string, extraRules []rbacv1.PolicyRule, log *logging.ReconcileLogger) error {
	rules := []rbacv1.PolicyRule{
		...
		{APIGroups: []string{"runs.bubustack.io"}, Resources: []string{"effectclaims"}, Verbs: []string{"get", "create", "update"}},
...
func (r *RBACManager) collectStoryRBACRules(ctx context.Context, storyRun *runsv1alpha1.StoryRun, story *bubuv1alpha1.Story, log *logging.ReconcileLogger) ([]rbacv1.PolicyRule, error) {
	...
	var templateRules []rbacv1.PolicyRule
	var overrideRules []rbacv1.PolicyRule

	for _, steps := range [][]bubuv1alpha1.Step{story.Spec.Steps, story.Spec.Compensations, story.Spec.Finally} {
		for i := range steps {
			step := &steps[i]
			if step.Ref == nil {
				continue
			...
			if template != nil && template.Spec.Execution != nil && template.Spec.Execution.RBAC != nil {
				templateRules = append(templateRules, template.Spec.Execution.RBAC.Rules...)
			}
			if engram.Spec.Overrides != nil && engram.Spec.Overrides.RBAC != nil {
				overrideRules = append(overrideRules, engram.Spec.Overrides.RBAC.Rules...)
			}
		}
	}

	if len(templateRules) == 0 && len(overrideRules) == 0 {
		return nil, nil
	}

	templateRules = mergePolicyRules(nil, templateRules)
	overrideRules = mergePolicyRules(nil, overrideRules)

	filteredTemplates := sanitizeTemplateStoryRBACRules(templateRules)
	filteredOverrides := sanitizeStoryRBACRules(overrideRules)
	filtered := mergePolicyRules(filteredTemplates, filteredOverrides)

	templateDropped := len(templateRules) - len(filteredTemplates)
	overrideDropped := len(overrideRules) - len(filteredOverrides)
	if templateDropped > 0 || overrideDropped > 0 {
		log.WithStoryRun(storyRun).Info(
			"Dropped unsafe dynamic RBAC rules",
			"droppedTemplate", templateDropped,
			"droppedOverrides", overrideDropped,
			"kept", len(filtered),
		)
	}
	return filtered, nil
...
func validateStoryRunServiceAccountOwnership(sa *corev1.ServiceAccount, storyRun *runsv1alpha1.StoryRun) error {
	if sa == nil || storyRun == nil || !existingObjectMeta(sa) {
		return nil
	}
	if hasStoryRunOwnerReference(sa.OwnerReferences, storyRun) {
		return nil
	}
	return fmt.Errorf(
		"refusing to adopt existing ServiceAccount %s/%s: owner reference does not match StoryRun %s",
		sa.Namespace,
		sa.Name,
		storyRun.Name,
	)
...
func hasStoryRunOwnerReference(refs []metav1.OwnerReference, storyRun *runsv1alpha1.StoryRun) bool {
	return matchesOwnerReference(refs, runsv1alpha1.GroupVersion.String(), "StoryRun", storyRun.Name, storyRun.UID, false)
}

func matchesOwnerReference(refs []metav1.OwnerReference, apiVersion, kind, name string, uid types.UID, requireController bool) bool {
	for _, ref := range refs {
		...
		if requireController && (ref.Controller == nil || !*ref.Controller) {
			continue
		}
		return true
	}
...
func existingObjectMeta(obj metav1.Object) bool {
	if obj == nil {
		return false
	}
	return obj.GetResourceVersion() != "" || obj.GetUID() != "" || !obj.GetCreationTimestamp().Time.IsZero()
}
...
func (r *RBACManager) emitRBACEvent(obj client.Object, reason, message string) {
	...
	r.Recorder.Eventf(obj, nil, corev1.EventTypeNormal, reason, "Reconcile", message)
}
...
func sanitizeStoryRBACRules(rules []rbacv1.PolicyRule) []rbacv1.PolicyRule {
	if len(rules) == 0 {
		return nil
	}
	filtered := make([]rbacv1.PolicyRule, 0, len(rules))
	for _, rule := range rules {
		if !isSafeStoryRBACRule(rule) {
			continue
		}
	...
	return mergePolicyRules(nil, filtered)
}

func sanitizeTemplateStoryRBACRules(rules []rbacv1.PolicyRule) []rbacv1.PolicyRule {
	if len(rules) == 0 {
		return nil
	}
	filtered := make([]rbacv1.PolicyRule, 0, len(rules))
	for _, rule := range rules {
		if !isTrustedTemplateStoryRBACRule(rule) {
			continue
		}
		filtered = append(filtered, rule)
	}
...
// containsEmptyOrWildcard returns true if any element in values is empty or "*" after trimming.
func containsEmptyOrWildcard(values []string) bool {
...
func isTrustedTemplateStoryRBACRule(rule rbacv1.PolicyRule) bool {
	...
	for _, apiGroup := range rule.APIGroups {
		if trimmed := strings.TrimSpace(apiGroup); trimmed == "*" {
			return false
		}
	}
	return !containsEmptyOrWildcard(rule.Resources) &&
		!containsEmptyOrWildcard(rule.Verbs) &&
		!containsEmptyOrWildcard(rule.ResourceNames)
}

func isSafeStoryRBACRule(rule rbacv1.PolicyRule) bool {
	if len(rule.NonResourceURLs) > 0 {
		return false
	}
	...
	for _, resource := range rule.Resources {
		...
		if _, ok := allowedStoryRBACResources[trimmed]; !ok {
			return false
		}
	}
	for _, verb := range rule.Verbs {
		trimmed := strings.TrimSpace(verb)
		if _, ok := allowedStoryRBACVerbs[trimmed]; !ok {
			return false
		}
	}
	return true

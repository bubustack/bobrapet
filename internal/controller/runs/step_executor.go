...
func (e *StepExecutor) Execute(ctx context.Context, srun *runsv1alpha1.StoryRun, story *bubuv1alpha1.Story, step *bubuv1alpha1.Step, vars map[string]any) (err error) {
	...
	ctx = withStoryMaxRecursionDepth(ctx, e.ConfigResolver, story)
...
func (e *StepExecutor) maybeOffloadStepRunInput(_ context.Context, srun *runsv1alpha1.StoryRun, stepName string, maxInlineBytes int, input *runtime.RawExtension) (*runtime.RawExtension, error) {
	...
	// Input exceeds inline limit — reject.
	// The controller does not auto-offload step inputs (see https://bubustack.io/docs/runtime/payloads).
	// Increase engram.default-max-inline-size or per-step execution.maxInlineSize.
	return nil, fmt.Errorf(
		"step input for %s is %d bytes which exceeds the max inline limit of %d bytes; "+
			"increase engram.default-max-inline-size in the operator ConfigMap or set execution.maxInlineSize on the step",
		stepName, len(input.Raw), maxInlineBytes,
	)
}
...
func (e *StepExecutor) resolveTemplateWith(ctx context.Context, srun *runsv1alpha1.StoryRun, purpose, stepName, childName string, raw *runtime.RawExtension, vars map[string]any, storyAnnotations map[string]string) (*runtime.RawExtension, error) {
	...
	offloadedDetect := detectOffloadedForPolicy(withVars, stepOutputs, policy, resolveAll)
	if offloadedDetect != nil {
		...
		if shouldBlockOffloaded(policy) && srun != nil {
			...
			materialized, matErr := resolveMaterialize(ctx, e.Client, e.Scheme, e.ConfigResolver, srun, purpose, stepName, childName, materializeModeObject, withVars, vars)
			if matErr != nil {
				return nil, matErr
			}
		...
		if shouldFailOnOffloaded(policy) {
			return nil, offloadedPolicyViolation("'with' block", policy, offloadedDetect)
		}
	...
	if err != nil {
		...
		if errors.As(err, &offloaded) {
			...
			if shouldFailOnOffloaded(policy) {
				return nil, offloadedPolicyViolation("'with' block", policy, offloaded)
			}

...
const triggerDataSecretKey = "triggerData"
const triggerDataHashAnnotation = "bubustack.io/trigger-data-hash"

func triggerDataSecretName(step *runsv1alpha1.StepRun) string {
	return step.Name + "-trigger-data"
}

func hashTriggerDataPayload(data []byte) string {
...
func (r *StepRunReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	defer func() {
		...
		requeue := res.RequeueAfter > 0 //nolint:staticcheck // res.Requeue check removed
		span.SetAttributes(
...
func (r *StepRunReconciler) reconcileNormal(ctx context.Context, step *runsv1alpha1.StepRun) (ctrl.Result, error) {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			if r.Recorder != nil {
				r.Recorder.Eventf(step, nil, "Warning", conditions.ReasonReferenceNotFound, "Reconcile", "Engram '%s' not found", step.Spec.EngramRef.Name)
			}
...
func (r *StepRunReconciler) guardStepRunReferences(ctx context.Context, step *runsv1alpha1.StepRun, log *logging.ControllerLogger) (bool, error) {
	...
	if err := webhookshared.ValidateCrossNamespaceReference(
		...
		if r.Recorder != nil {
			r.Recorder.Eventf(step, nil, corev1.EventTypeWarning, conditions.ReasonStoryReferenceInvalid, "Reconcile", message)
		}
	...
	if err := webhookshared.ValidateCrossNamespaceReference(
		...
		if r.Recorder != nil {
			r.Recorder.Eventf(step, nil, corev1.EventTypeWarning, conditions.ReasonEngramReferenceInvalid, "Reconcile", message)
		}
...
func (r *StepRunReconciler) assembleJobResources(ctx context.Context, in jobResourceInputs) (jobResources, error) {
	...
	if tlsSecret := getTLSSecretName(&in.engram.ObjectMeta); tlsSecret != "" {
		if err := r.configureTLSEnvAndMounts(ctx, in.engram.Namespace, tlsSecret, &volumes, &volumeMounts, &envVars); err != nil {
			...
			return jobResources{}, fmt.Errorf("failed to configure TLS for StepRun job: %w", err)
		}
	}

	r.addStartedAtEnv(&envVars, resolveStepRunStartedAt(in.srun))
...
func (r *StepRunReconciler) ensureStorageSecret(ctx context.Context, targetNamespace string, resolved *config.ResolvedExecutionConfig, logger *logging.ControllerLogger) error {
	...
	auth := defaultStorageSecretCopyAuthorization(r.ConfigResolver, secretName)
	if err := kubeutil.EnsureSecretCopy(ctx, reader, r.Client, sourceNamespace, targetNamespace, secretName, auth); err != nil {
		err = fmt.Errorf("cross-namespace storage secret sync denied: %w", err)
...
func (r *StepRunReconciler) buildBaseEnvVars(srun *runsv1alpha1.StepRun, story *v1alpha1.Story, inputBytes []byte, stepTimeout time.Duration, executionMode string, resolvedConfig *config.ResolvedExecutionConfig, debugEnabled bool) []corev1.EnvVar {
	...
	maxRecursionDepth := config.ResolveStoryMaxRecursionDepth(story, config.EffectiveMaxRecursionDepth(operatorCfg))
	...
	envVars := []corev1.EnvVar{
		...
		{Name: contracts.MaxRecursionDepthEnv, Value: strconv.Itoa(maxRecursionDepth)},
...
func resolveStepRunStartedAt(step *runsv1alpha1.StepRun) metav1.Time {
	if step == nil {
		return metav1.NewTime(time.Now().UTC())
	}
	if step.Status.StartedAt != nil {
		return metav1.NewTime(step.Status.StartedAt.UTC())
	}
	if !step.CreationTimestamp.IsZero() {
		return metav1.NewTime(step.CreationTimestamp.UTC())
	}
	return metav1.NewTime(time.Now().UTC())
}
...
//
// Behavior:
//   - Applies Story input defaults.
//   - Hydrates offloaded StoryRun input storage refs before controller-side template evaluation.
//
// Arguments:
...
func (r *StepRunReconciler) getStoryRunInputs(ctx context.Context, storyRun *runsv1alpha1.StoryRun, story *v1alpha1.Story) (map[string]any, error) {
	ctx = withStoryMaxRecursionDepth(ctx, r.ConfigResolver, story)
	return resolveHydratedStoryRunInputs(ctx, storyRun, story)
}
...
func (r *StepRunReconciler) ensureStepRunJobUID(ctx context.Context, step *runsv1alpha1.StepRun, job *batchv1.Job, log *logging.ControllerLogger) {
	...
	if prevUID != "" {
		...
		if r.Recorder != nil {
			r.Recorder.Eventf(step, nil, corev1.EventTypeWarning, eventReasonJobRestarted, "Reconcile", "StepRun Job UID changed (previous=%s new=%s)", prevUID, jobUID)
		}
		if log != nil {
...
func (r *StepRunReconciler) markStepRunRestart(ctx context.Context, step *runsv1alpha1.StepRun, reason string, log *logging.ControllerLogger) error {
	...
	if r.Recorder != nil {
		r.Recorder.Eventf(step, nil, corev1.EventTypeWarning, eventReasonJobRestarted, "Reconcile", "Restarting StepRun after Job went missing (reason=%s previousJobUID=%s)", reason, prevUID)
	}
	if log != nil {
...
func (r *StepRunReconciler) reconcileRunScopedRealtimeStep(ctx context.Context, step *runsv1alpha1.StepRun, story *v1alpha1.Story, stepSpec *v1alpha1.Step, engram *v1alpha1.Engram, template *catalogv1alpha1.EngramTemplate) (ctrl.Result, error) {
	...
	allowInsecure := false // Secure transport by default
...
func (r *StepRunReconciler) resolveRunScopedInputs(ctx context.Context, step *runsv1alpha1.StepRun, storyRun *runsv1alpha1.StoryRun, story *v1alpha1.Story, inputSchema *runtime.RawExtension, schemaName string) ([]byte, error) {
	ctx = withStoryMaxRecursionDepth(ctx, r.ConfigResolver, story)
	if schemaName == "" {
		schemaName = "Step inputs" //nolint:goconst
	}
	...
	if isMaterializeStepRun(step) {
		...
		offloadedDetect := detectOffloadedForPolicy(withBlock, stepOutputsMap, policy, resolveAll)
		if offloadedDetect != nil {
			if shouldFailOnOffloaded(policy) {
				return nil, offloadedPolicyViolation("step input", policy, offloadedDetect)
			}
			if resolveAll {
				...
				materialized, matErr := resolveMaterialize(ctx, r.Client, r.Scheme, r.ConfigResolver, storyRun, materializePurposeStepInput, step.Spec.StepID, "", materializeModeObject, withBlock, vars)
				if matErr != nil {
					var blocked *templating.ErrEvaluationBlocked
		...
		if resolvedInputs == nil {
			...
			if err != nil {
				...
				if errors.As(err, &offloaded) {
					if shouldFailOnOffloaded(policy) {
						return nil, offloadedPolicyViolation("step input", policy, offloaded)
					}
					if resolveAll {
...
func defaultStorageSecretCopyAuthorization(resolver *config.Resolver, secretName string) kubeutil.SecretCopyAuthorization {
	if resolver == nil {
		return kubeutil.SecretCopyAuthorization{}
	}
	cfg := resolver.GetOperatorConfig()
	if cfg == nil {
		return kubeutil.SecretCopyAuthorization{}
	}
	defaultName := strings.TrimSpace(cfg.Controller.DefaultS3AuthSecretName)
	if defaultName == "" || secretName != defaultName {
		return kubeutil.SecretCopyAuthorization{}
	}
	return kubeutil.AllowSecretCopyNames(secretName)
}
...
func (r *StepRunReconciler) maybeWriteCache(ctx context.Context, step *runsv1alpha1.StepRun, log *logging.ControllerLogger) {
	...
	if rawTTL := strings.TrimSpace(step.Annotations[cacheAnnotationTTL]); rawTTL != "" {
		if parsed, err := strconv.Atoi(rawTTL); err == nil && parsed >= 0 && parsed <= math.MaxInt32 {
			val := int32(parsed)
			ttl = &val
		}
	}
...
func (r *StepRunReconciler) ensureRunTransportBinding(
	...
	if err := r.ensureBindingRoutingStatus(ctx, binding, storyRun, stepSpec, ctxData); err != nil {
		ctxData.logger.Error(err, "Failed to update transport binding routing status", "binding", binding.Name)
	}
...
func (r *StepRunReconciler) buildRealtimeBaseEnv(
	...
	bindingEnv string,
	bindingInfo *transportpb.BindingInfo,
	_ bool, // allowInsecure reserved for future TLS policy
	stepTimeout time.Duration,
	runtimeEnvCfg coreenv.Config,
	operatorCfg *config.OperatorConfig,
) ([]corev1.EnvVar, error) {
	...
	upsertEnvVar(&baseEnv, corev1.EnvVar{
		Name:  contracts.MaxRecursionDepthEnv,
		Value: strconv.Itoa(config.ResolveStoryMaxRecursionDepth(story, config.EffectiveMaxRecursionDepth(operatorCfg))),
	})

	coreenv.AppendStartedAtEnv(&baseEnv, resolveStepRunStartedAt(step))
	...
	if transportsJSON := r.marshalStoryTransports(ctx, story); transportsJSON != "" {
	...
	baseEnv = transportutil.ApplyTransportSecurityEnv(baseEnv)
	if stepTimeout > 0 {
...
func (r *StepRunReconciler) marshalStoryTransports(ctx context.Context, story *v1alpha1.Story) string {
	if story == nil {
		return ""
	}
	...
	payload, err := transportutil.MarshalRuntimeTransportDescriptors(ctx, reader, story)
	if err != nil {
		return ""
	}
	return payload
...
// Behavior:
//   - Applies Story input defaults.
//   - Hydrates offloaded StoryRun input storage refs before marshaling.
//
// Arguments:
...
func (r *StepRunReconciler) getStoryRunInputsBytes(ctx context.Context, storyRun *runsv1alpha1.StoryRun, story *v1alpha1.Story) ([]byte, error) {
	ctx = withStoryMaxRecursionDepth(ctx, r.ConfigResolver, story)
	return resolveHydratedStoryRunInputsBytes(ctx, storyRun, story)
}
...
func hasDebugAnnotation(annotations map[string]string) bool {
	...
	case "1", "true", "yes", "y", "on": //nolint:goconst
		return true
	default:
		return false
	}
...
func (r *StepRunReconciler) SetupWithManager(mgr ctrl.Manager, opts controller.Options) error {
	r.Recorder = mgr.GetEventRecorder("steprun-controller")
...
func (r *StepRunReconciler) configureTLSEnvAndMounts(
	...
	appendEnvIfMissing(envVars, corev1.EnvVar{Name: contracts.GRPCCAFileEnv, Value: mountPath + "/ca.crt"})

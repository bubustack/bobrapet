...
// listWarningEvents returns all corev1.Event objects with type Warning in the
// fake client's store.
func listWarningEvents(t *testing.T, ctx context.Context, cl *fake.ClientBuilder, reconciler *StepRunReconciler) []corev1.Event {
	t.Helper()
	var events corev1.EventList
	require.NoError(t, reconciler.Client.List(ctx, &events))

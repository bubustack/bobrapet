...
var _ = Describe("Story CRD validations", func() {
	...
	It("rejects duplicate step names", func() {
		...
		_, err := validator.ValidateCreate(context.Background(), story.DeepCopy())
	...
	It("rejects cycles by detecting dependency loops", func() {
		story := &bubuv1alpha1.Story{
			ObjectMeta: metav1.ObjectMeta{
...
var _ = Describe("StepRun CRD validations", func() {
	...
	It("rejects status.needs entries referencing the StepRun itself", func() {
		...
		_, err := validator.ValidateCreate(context.Background(), stepRun.DeepCopy())
		Expect(err).NotTo(HaveOccurred())

		oldObj := stepRun.DeepCopy()
		newStepRun := stepRun.DeepCopy()
		newStepRun.Status.Needs = []string{"self-referencing-step-run"}

		_, err = validator.ValidateUpdate(context.Background(), oldObj, newStepRun.DeepCopy())
		Expect(err).To(HaveOccurred())

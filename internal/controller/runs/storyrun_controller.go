...
// +kubebuilder:rbac:groups=rbac.authorization.k8s.io,resources=roles,verbs=create;get;watch;list;update;patch;escalate;bind
// +kubebuilder:rbac:groups=rbac.authorization.k8s.io,resources=rolebindings,verbs=create;get;watch;list;update;patch;delete
	...
	for stepName, childNames := range srun.Status.PrimitiveChildren {
		...
		for _, childName := range childNames {
			...
			if err := r.Get(ctx, key, &child); err != nil {
				...
				meta.Error(log.Logr(), err, "Failed to fetch child StoryRun during redrive-from-step", "childStoryRun", childName)
				contract.Failure("cleanup", err, "phase", "get-storyrun", "childStoryRun", childName)
				return cleanupSummary{remaining: true}, err
			}
			summary.remaining = true
...
func (r *StoryRunReconciler) guardStoryReference(ctx context.Context, srun *runsv1alpha1.StoryRun, log *logging.ControllerLogger) (bool, error) {
	...
	if err := webhookshared.ValidateCrossNamespaceReference(
		...
		if r.Recorder != nil {
			r.Recorder.Event(srun, "Warning", conditions.ReasonStoryReferenceInvalid, message)
		}
...
func (r *StoryRunReconciler) validateStoryRunInputs(ctx context.Context, srun *runsv1alpha1.StoryRun, story *bubushv1alpha1.Story, log *logging.ControllerLogger) (bool, error) {
	...
	resolved, err := runsinputs.ResolveStoryRunInputs(story, srun)
	if err != nil {

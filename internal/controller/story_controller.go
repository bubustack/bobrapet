...
func (r *StoryReconciler) ensureStoryFinalizer(ctx context.Context, story *bubuv1alpha1.Story, logger logr.Logger) error {
	...
	if err != nil {
		...
		if r.Recorder != nil {
			r.Recorder.Eventf(story, nil, "Warning", "FinalizerAddFailed", "Reconcile", err.Error())
		}
		return err
	}
	if !added {
	...
	if r.Recorder != nil {
		r.Recorder.Eventf(story, nil, "Normal", "FinalizerAdded", "Reconcile", "Story finalizer attached")
	}
	return nil
}
...
func (r *StoryReconciler) reconcileDelete(ctx context.Context, story *bubuv1alpha1.Story) (ctrl.Result, error) {
	...
	if err := r.Patch(ctx, story, mergePatch); err != nil {
		...
		if r.Recorder != nil {
			r.Recorder.Eventf(story, nil, "Warning", "FinalizerRemovalFailed", "Reconcile", err.Error())
		}
		return ctrl.Result{}, err
	}

	if r.Recorder != nil {
		r.Recorder.Eventf(story, nil, "Normal", "FinalizerRemoved", "Reconcile", "Story finalizer removed")
	}
	return ctrl.Result{}, nil
}
...
func (r *StoryReconciler) validateExecuteStoryStep(ctx context.Context, story *bubuv1alpha1.Story, step *bubuv1alpha1.Step, index int) error {
	...
	if story.Spec.Pattern == enums.BatchPattern && subStory.Spec.Pattern == enums.RealtimePattern {
		return validation.WrapWithReason(conditions.ReasonStoryReferenceInvalid,
...
func (r *StoryReconciler) SetupWithManager(mgr ctrl.Manager, opts controller.Options) error {
	r.Recorder = mgr.GetEventRecorder("story-controller")

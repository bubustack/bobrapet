...
import (
	...
	transportcap "github.com/bubustack/bobrapet/pkg/transport/capabilities"
...
func (r *TransportReconciler) collectAvailableCapabilities(
	...
	snapshot, err := transportcap.AggregateBindings(
		ctx,
		r.Client,
		transport,
		transportcap.AggregationOptions{
			Logger:           logger,
...
func (r *TransportReconciler) updateStatus(
	...
	pendingBindings := totalBindings - readyBindings - failedBindings
	...
	err := kubeutil.RetryableStatusPatch(ctx, r.Client, transport, func(obj client.Object) {
		...
		if len(availableAudio) == 0 {
			availableAudio = transportcap.CloneAudioCodecs(transport.Spec.SupportedAudio)
		}
		if len(availableVideo) == 0 {
			availableVideo = transportcap.CloneVideoCodecs(transport.Spec.SupportedVideo)
		}
		if len(availableBinary) == 0 {
			availableBinary = transportcap.CloneBinaryCapabilities(transport.Spec.SupportedBinary)
		}
		t.Status.AvailableAudio = transportcap.CloneAudioCodecs(availableAudio)
		t.Status.AvailableVideo = transportcap.CloneVideoCodecs(availableVideo)
		t.Status.AvailableBinary = transportcap.CloneBinaryCapabilities(availableBinary)

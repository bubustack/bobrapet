...
type TransportReconciler struct {
	...
	warningEventsMu          sync.Mutex
	warningEventFingerprints map[string]string
}
...
func (r *TransportReconciler) updateStatus(
	...
	validationMessages := normalizeBindingMessages(validationErrors)
	validationSummary := strings.Join(validationMessages, "; ")
	...
	if validationFingerprint := warningEventFingerprint(validationMessages); validationFingerprint == "" {
		r.clearWarningEventFingerprint(transport.Name, conditions.ReasonValidationFailed)
	} else if strings.TrimSpace(validationSummary) != "" &&
		r.shouldEmitWarningEvent(transport.Name, conditions.ReasonValidationFailed, validationFingerprint) {
	...
	if bindingFingerprint := warningEventFingerprint(bindingErrors); bindingFingerprint == "" {
		r.clearWarningEventFingerprint(transport.Name, conditions.ReasonTransportFailed)
	} else if strings.TrimSpace(bindingSummary) != "" &&
		r.shouldEmitWarningEvent(transport.Name, conditions.ReasonTransportFailed, bindingFingerprint) {
		kubeutil.RecordEvent(
			r.Recorder,
			log.Logr(),
			transport,
...
// warningEventFingerprint builds a stable dedupe key for warning events by
// normalizing and sorting message content, making event emission insensitive to
// non-semantic ordering changes.
func warningEventFingerprint(messages []string) string {
	cleaned := normalizeBindingMessages(messages)
	if len(cleaned) == 0 {
		return ""
	}
	stable := append([]string(nil), cleaned...)
	sort.Strings(stable)
	return strings.Join(stable, "\n")
}

func (r *TransportReconciler) shouldEmitWarningEvent(transportName, reason, fingerprint string) bool {
	transportName = strings.TrimSpace(transportName)
	reason = strings.TrimSpace(reason)
	fingerprint = strings.TrimSpace(fingerprint)
	if transportName == "" || reason == "" || fingerprint == "" {
		return false
	}

	key := transportWarningEventKey(transportName, reason)

	r.warningEventsMu.Lock()
	defer r.warningEventsMu.Unlock()

	if previous, found := r.warningEventFingerprints[key]; found && previous == fingerprint {
		return false
	}

	if r.warningEventFingerprints == nil {
		r.warningEventFingerprints = make(map[string]string)
	}
	r.warningEventFingerprints[key] = fingerprint
	return true
}

func (r *TransportReconciler) clearWarningEventFingerprint(transportName, reason string) {
	transportName = strings.TrimSpace(transportName)
	reason = strings.TrimSpace(reason)
	if transportName == "" || reason == "" {
		return
	}

	r.warningEventsMu.Lock()
	defer r.warningEventsMu.Unlock()
	if len(r.warningEventFingerprints) == 0 {
		return
	}
	delete(r.warningEventFingerprints, transportWarningEventKey(transportName, reason))
}

func transportWarningEventKey(transportName, reason string) string {
	return transportName + "|" + reason
}
...
func (r *TransportReconciler) SetupWithManager(mgr ctrl.Manager, opts controller.Options) error {
	...
	r.Recorder = mgr.GetEventRecorder("transport-controller")
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			&bubuv1alpha1.Story{},
			handler.EnqueueRequestsFromMapFunc(r.storyToTransport),
			builder.WithPredicates(transportStoryWatchPredicate()),
		).
		WithOptions(opts).
		Complete(r)
}

func transportStoryWatchPredicate() predicate.Predicate {

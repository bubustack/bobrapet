...
func TestTransportStoryWatchPredicateIgnoresStatusOnlyStoryUpdates(t *testing.T) {
	t.Parallel()

	pred := transportStoryWatchPredicate()
	oldStory := &bubuv1alpha1.Story{
	...
	statusOnlyUpdate := oldStory.DeepCopy()
	statusOnlyUpdate.Status.ValidationStatus = enums.ValidationStatusInvalid
	statusOnlyUpdate.Status.ValidationErrors = []string{"missing transport"}

	require.False(t, pred.Update(event.UpdateEvent{ObjectOld: oldStory, ObjectNew: statusOnlyUpdate}))

	specUpdate := statusOnlyUpdate.DeepCopy()
	specUpdate.Generation++
	require.True(t, pred.Update(event.UpdateEvent{ObjectOld: statusOnlyUpdate, ObjectNew: specUpdate}))
}
...
func TestTransportReconciler_updateStatusEmitsBothEventsAndSummaries(t *testing.T) {
	...
	recEvents := readAllRecorderEvents(recorder)
	require.Len(t, recEvents, 2)
	require.Contains(t, recEvents[0], conditions.ReasonValidationFailed)
	require.Contains(t, recEvents[1], conditions.ReasonTransportFailed)
	require.Contains(t, recEvents[1], "... (+1 more)")
...
func TestTransportReconciler_updateStatusDedupesWarningsAndReEmitsAfterRecovery(t *testing.T) {
	t.Parallel()
	...
	validationErrs := []string{"driver missing"}
	bindingErrs := []string{"default/binding: heartbeat stale"}

	err := reconciler.updateStatus(context.Background(), transport, validationErrs, 0, nil, nil, nil, 0, 1, bindingErrs, nil)
	require.NoError(t, err)
	first := readAllRecorderEvents(recorder)
	require.Len(t, first, 2)

	err = reconciler.updateStatus(context.Background(), transport, validationErrs, 0, nil, nil, nil, 0, 1, bindingErrs, nil)
	require.NoError(t, err)
	require.Empty(t, readAllRecorderEvents(recorder))

	err = reconciler.updateStatus(context.Background(), transport, nil, 0, nil, nil, nil, 1, 1, nil, nil)
	require.NoError(t, err)
	require.Empty(t, readAllRecorderEvents(recorder))

	err = reconciler.updateStatus(context.Background(), transport, validationErrs, 0, nil, nil, nil, 0, 1, bindingErrs, nil)
	require.NoError(t, err)
	second := readAllRecorderEvents(recorder)
	require.Len(t, second, 2)
}

func TestTransportReconciler_updateStatusReEmitsBindingWarningWhenHiddenErrorsChange(t *testing.T) {
	t.Parallel()
	...
	err := reconciler.updateStatus(context.Background(), transport, nil, 0, nil, nil, nil, 0, 6, bindingErrsA, nil)
	require.NoError(t, err)
	first := readAllRecorderEvents(recorder)
	require.Len(t, first, 1)
	require.Contains(t, first[0], conditions.ReasonTransportFailed)
	require.Contains(t, first[0], "... (+1 more)")

	err = reconciler.updateStatus(context.Background(), transport, nil, 0, nil, nil, nil, 0, 6, bindingErrsB, nil)
	require.NoError(t, err)
	second := readAllRecorderEvents(recorder)
	require.Len(t, second, 1)
	require.Contains(t, second[0], conditions.ReasonTransportFailed)
}

func readRecorderEvent(recorder *events.FakeRecorder) string {
	select {
...
func readAllRecorderEvents(recorder *events.FakeRecorder) []string {
	var recEvents []string
	for {
		select {
		case evt := <-recorder.Events:
			recEvents = append(recEvents, evt)
		default:
			return recEvents
		}
	}

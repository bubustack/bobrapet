...
// CELLogger provides specialized logging for CEL operations
type CELLogger struct {
	*ControllerLogger
}

// NewCELLogger creates a logger for CEL operations
func NewCELLogger(ctx context.Context) *CELLogger {
	cl := NewControllerLogger(ctx, "cel")
	return &CELLogger{ControllerLogger: cl}
}

// EvaluationStart logs CEL evaluation start
func (cl *CELLogger) EvaluationStart(expression, expressionType string) {
	cl.V(2).Info("CEL evaluation started",
		"expression", expression,
		"type", expressionType)
}

// EvaluationSuccess logs CEL evaluation success
func (cl *CELLogger) EvaluationSuccess(expression, expressionType string, duration time.Duration, result interface{}) {
	cl.V(1).Info("CEL evaluation succeeded",
		"expression", expression,
		"type", expressionType,
...
// EvaluationError logs CEL evaluation error
func (cl *CELLogger) EvaluationError(err error, expression, expressionType string, duration time.Duration) {
	cl.Error(err, "CEL evaluation failed",
		"expression", expression,
		"type", expressionType,
		"duration", duration.String())
}

// CacheHit logs CEL cache hit
func (cl *CELLogger) CacheHit(expression, expressionType string) {
	cl.V(2).Info("CEL cache hit",
		"expression", expression,
		"type", expressionType)
}

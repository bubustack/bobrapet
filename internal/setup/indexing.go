...
func SetupIndexers(ctx context.Context, mgr manager.Manager) error {
	...
	if err := EnsureIndexes(ctx, mgr, allIndexRegistrations); err != nil {
		return fmt.Errorf("failed to register field indexes: %w", err)
	}
...
func collectStoryStepReferenceKeys(
	...
	for _, steps := range [][]bubushv1alpha1.Step{
		story.Spec.Steps,
		story.Spec.Compensations,
		story.Spec.Finally,
	} {
		for i := range steps {
			collect(story, &steps[i], nameSet)
		}
	}
...
// extractStepRunPhase exposes status.phase so reconcilers can count running StepRuns
// without scanning the entire collection.
func extractStepRunPhase(rawObj client.Object) []string {
	stepRun := rawObj.(*runsv1alpha1.StepRun)
	phase := strings.TrimSpace(string(stepRun.Status.Phase))
	if phase == "" {
		return nil
	}
	return []string{phase}

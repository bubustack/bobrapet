...
// LogRegisteredIndexes emits a summary log of all successfully registered indexes.
//
// Behavior:
//   - Iterates through allIndexRegistrations and logs each field name.
//   - Logs at V(1) level for normal operation, Info level for the summary.
//   - Helps operators verify index setup and troubleshoot fallback issues.
//
// Side Effects:
//   - Writes log entries to the setup logger.
func LogRegisteredIndexes() {
	fields := make([]string, 0, len(allIndexRegistrations))
	for _, reg := range allIndexRegistrations {
		fields = append(fields, reg.Field)
	}
	sort.Strings(fields)

	setupLog.Info("field indexes registered successfully",
		"count", len(fields),
		"indexes", fields,
	)
}
...
func EnsureIndexes(ctx context.Context, mgr manager.Manager, regs []IndexRegistration) error {
	for _, reg := range regs {
		if err := ensureIndex(ctx, mgr, reg); err != nil {
			return fmt.Errorf("%s: %w", reg.ErrMsg, err)
		}
	}
	return nil
}
...
func ensureIndex(ctx context.Context, mgr manager.Manager, reg IndexRegistration) error {
	...
	if _, exists := registeredIndexes.LoadOrStore(key, struct{}{}); exists {
		return nil
	}
	return indexFieldWithRetry(ctx, mgr, reg.Object, reg.Field, reg.Extractor)
}
...
// extractStepRunStoryRunRef returns the StepRun's spec.storyRunRef.name for
// indexing, emitting nil when the reference is unset
...
//   - Emits a single-element slice containing only the StoryRun name; note
//     that namespace data is currently discarded.
//
// Arguments:
...
//     referenced StoryRun name.
//
// Side Effects:
//   - None.
//
// Notes / Gotchas:
//   - Does not include namespaces, so cross-namespace StoryRun references would
//     collide until this helper adopts refs.ResolveNamespace.
func extractStepRunStoryRunRef(rawObj client.Object) []string {
	stepRun := rawObj.(*runsv1alpha1.StepRun)
	return indexReferenceName(&stepRun.Spec.StoryRunRef.ObjectReference)
}

// extractStoryRunImpulseRef exposes spec.impulseRef.name so StoryRuns can be
...
//     singleton slice with the referenced Impulse (internal/setup/indexing.go:230-236).
//
// Arguments:
...
//     containing the Impulse name.
//
// Side Effects:
//   - None.
//
// Notes / Gotchas:
//   - Currently drops namespace information even though refs.ImpulseReference
//     embeds refs.ObjectReference; consider adding namespace resolution if
//     cross-namespace triggers are supported.
...
// extractImpulseStoryRefName exposes an Impulse's spec.storyRef.name so Stories
// can enqueue dependent Impulses via a field index
// (internal/setup/indexing.go:62,317-320).
//
// Behavior:
//   - Returns nil when StoryRef.Name is empty.
//   - Emits a single-element slice with the Story name (namespace currently
//     omitted).
//
// Arguments:
...
//
// Returns:
//   - []string: nil or one entry containing the Story name.
//
// Side Effects:
//   - None.
//
// Notes / Gotchas:
//   - Namespace is ignored, so callers must filter cross-namespace references.
func extractImpulseStoryRefName(rawObj client.Object) []string {
	impulse := rawObj.(*bubushv1alpha1.Impulse)
	return indexReferenceName(&impulse.Spec.StoryRef.ObjectReference)
}
...
func extractStoryStepEngramRefs(rawObj client.Object) []string {
	...
	return collectStoryStepReferenceKeys(story, func(
		...
		if ref := loopTemplateRef(story, step); ref != nil {
			addNamespacedReference(nameSet, story, &ref.ObjectReference)
		}
	})
}

type loopTemplateConfig struct {
	Template bubushv1alpha1.Step `json:"template"`
}

func loopTemplateRef(story *bubushv1alpha1.Story, step *bubushv1alpha1.Step) *refs.EngramReference {
	if step == nil || step.With == nil || len(step.With.Raw) == 0 {
		return nil
	}
	var cfg loopTemplateConfig
	if err := json.Unmarshal(step.With.Raw, &cfg); err != nil {
		setupLog.V(1).Info(
			"Skipping loop step with invalid with JSON",

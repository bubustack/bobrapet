...
var _ = Describe("StepRun Webhook", func() {
	...
	Context("When creating or updating StepRun under Validating Webhook", func() {
		...
		It("returns error when oldObj is wrong type in ValidateUpdate", func() {
			obj = newStepRun(nil)
			_, err := validator.ValidateUpdate(context.Background(), &bubuv1alpha1.Engram{}, obj)
			Expect(err).To(MatchError(ContainSubstring("expected a StepRun object for the oldObj")))
		})
	...
	Context("Structured error status validation", func() {
		It("accepts valid structured errors", func() {
			sr := newStepRun(nil)
			sr.Status.Error = &runtime.RawExtension{
				Raw: []byte(`{"version":"v1","type":"timeout","message":"boom","retryable":true}`),
			}
		...
		It("rejects invalid structured errors", func() {
			sr := newStepRun(nil)
			sr.Status.Error = &runtime.RawExtension{
				Raw: []byte(`{"type":"timeout","message":"missing version"}`),
			}
			_, err := validator.ValidateCreate(context.Background(), sr)
			Expect(err).To(HaveOccurred())
		...
		It("rejects non-object error payloads", func() {
			sr := newStepRun(nil)
			sr.Status.Error = &runtime.RawExtension{
				Raw: []byte(`["nope"]`),
			}

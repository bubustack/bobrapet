...
var _ = Describe("StoryRun Webhook", func() {
	...
	Context("trigger tokens", func() {
		...
		It("accepts legacy deterministic name when trigger token annotation is set", func() {
			story := &bubuv1alpha1.Story{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "token-story-legacy",
					Namespace: "default",
				},
				Spec: bubuv1alpha1.StorySpec{
					Steps: []bubuv1alpha1.Step{{
			...
			inputHash, err := runsidentity.ComputeTriggerInputHashFromRawExtension(nil)
			Expect(err).NotTo(HaveOccurred())
			expectedLegacy := runsidentity.DeriveStoryRunName("", "token-story-legacy", token)
			sr := &runsv1alpha1.StoryRun{
				ObjectMeta: metav1.ObjectMeta{
					Namespace: "default",
					Name:      expectedLegacy,

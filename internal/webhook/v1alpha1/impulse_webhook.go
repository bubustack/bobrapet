...
func (wh *ImpulseWebhook) SetupWebhookWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewWebhookManagedBy(mgr).For(&bubushv1alpha1.Impulse{}).
		WithValidator(&ImpulseCustomValidator{Client: wh.Client, Config: wh.Config, ConfigManager: wh.ConfigManager}).
...
func (v *ImpulseCustomValidator) ValidateCreate(ctx context.Context, obj runtime.Object) (admission.Warnings, error) {
	...
	if !ok {
		return nil, fmt.Errorf("expected an Impulse object but got %T", obj)
	}
...
func (v *ImpulseCustomValidator) ValidateUpdate(ctx context.Context, oldObj, newObj runtime.Object) (admission.Warnings, error) {
	...
	if !ok {
		return nil, fmt.Errorf("expected an Impulse object for the newObj but got %T", newObj)
	}
	...
	if impulse.DeletionTimestamp != nil {
		return nil, nil
	}
	...
	if oldImpulse, ok := oldObj.(*bubushv1alpha1.Impulse); ok {
		if reflect.DeepEqual(normalizeImpulseSpec(oldImpulse.Spec), normalizeImpulseSpec(impulse.Spec)) {
			return nil, nil
		}
	}
...
func (v *ImpulseCustomValidator) validateImpulse(ctx context.Context, impulse *bubushv1alpha1.Impulse) (admission.Warnings, error) {
	...
	if impulse.Spec.StoryRef.Name != "" && v.Client != nil {
		...
		if err := v.Client.Get(storyCtx, storyKey, story); err != nil {
...
func (v *ImpulseCustomValidator) fetchTemplate(ctx context.Context, name string) (*v1alpha1.ImpulseTemplate, error) {
	...
	if err := v.Client.Get(webhookCtx, types.NamespacedName{Name: name, Namespace: ""}, &template); err != nil {
		if apierrors.IsNotFound(err) {

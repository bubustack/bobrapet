...
func (wh *StoryWebhook) SetupWebhookWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewWebhookManagedBy(mgr, &bubushv1alpha1.Story{}).
		WithValidator(&StoryCustomValidator{
			Client:        wh.Client,
			APIReader:     wh.APIReader,
			Config:        wh.Config,
...
var _ admission.Defaulter[*bubushv1alpha1.Story] = &StoryCustomDefaulter{}
...
func (d *StoryCustomDefaulter) Default(_ context.Context, story *bubushv1alpha1.Story) error {
	storylog.Info("Defaulting Story", "name", story.GetName())
...
var _ admission.Validator[*bubushv1alpha1.Story] = &StoryCustomValidator{}
...
func (v *StoryCustomValidator) lookupReader() client.Reader {
	if v.APIReader != nil {
		return v.APIReader
	}
	return v.Client
...
func (v *StoryCustomValidator) validateStory(ctx context.Context, story *bubushv1alpha1.Story) (admission.Warnings, error) {
	...
	// Compensations and finally blocks are allowed for both batch and realtime stories.
	// For realtime stories, cleanup steps execute as batch jobs after the realtime
...
func (sv *stepShapeValidator) checkExpressions(story *bubushv1alpha1.Story, step *bubushv1alpha1.Step, stepPath string, agg *validation.Aggregator) {
	...
	staticScope := realtimeStaticScope()
	...
	if step.Runtime != nil && len(step.Runtime.Raw) > 0 {
		runtimeAllowed := story.Spec.Pattern.IsRealtime()
		var warning string
		if !runtimeAllowed {
			runtimeAllowed, warning = sv.runtimeAllowedForBatchStory(story, step)
		}
		if !runtimeAllowed {
			agg.AddFieldError(stepPath+".runtime", conditions.ReasonValidationFailed, "runtime is only supported for realtime stories or bridged realtime steps")
		} else {
			if warning != "" {
...
func (sv *stepShapeValidator) runtimeAllowedForBatchStory(story *bubushv1alpha1.Story, step *bubushv1alpha1.Step) (bool, string) {
	...
	return true, fmt.Sprintf("step '%s': runtime allowed for batch story because engram '%s/%s' was not found to confirm realtime mode", step.Name, engramKey.Namespace, engramKey.Name)
}
...
func realtimeStaticScope() templating.ExpressionScope {
	return templating.NewExpressionScope("realtime-static", false, false, true, templating.RootInputs)
}

func realtimeRuntimeScope() templating.ExpressionScope {
	return templating.NewExpressionScope("realtime-runtime", true, true, true, templating.RootInputs, templating.RootSteps, templating.RootPacket)
}
...
func validateStoryTransportRoutingRules(story *bubushv1alpha1.Story, agg *validation.Aggregator) {
	...
	for i := range story.Spec.Transports {
		...
		for j, rule := range decl.Streaming.Routing.Rules {
			...
			if rule.Action != "" && rule.Action != transportv1alpha1.TransportRoutingRuleAllow && rule.Action != transportv1alpha1.TransportRoutingRuleDeny {
	...
	for i := range story.Spec.Transports {
		...
		if streaming.Backpressure != nil && streaming.Backpressure.Buffer != nil {
			...
			if strings.TrimSpace(string(buf.DropPolicy)) != "" &&
				buf.DropPolicy != transportv1alpha1.BufferDropNewest && buf.DropPolicy != transportv1alpha1.BufferDropOldest {
		...
		if streaming.FlowControl != nil {
			...
			if strings.TrimSpace(string(flow.Mode)) != "" &&
				flow.Mode != transportv1alpha1.FlowControlNone && flow.Mode != transportv1alpha1.FlowControlCredits && flow.Mode != transportv1alpha1.FlowControlWindow {
		...
		if streaming.Delivery != nil {
			...
			if strings.TrimSpace(string(delivery.Ordering)) != "" &&
				delivery.Ordering != transportv1alpha1.OrderingNone && delivery.Ordering != transportv1alpha1.OrderingPerStream && delivery.Ordering != transportv1alpha1.OrderingPerPartition {
			...
			if strings.TrimSpace(string(delivery.Semantics)) != "" &&
				delivery.Semantics != transportv1alpha1.DeliveryBestEffort && delivery.Semantics != transportv1alpha1.DeliveryAtLeastOnce {
			...
			if delivery.Replay != nil {
				...
				if strings.TrimSpace(string(replay.Mode)) != "" &&
					replay.Mode != transportv1alpha1.ReplayNone && replay.Mode != transportv1alpha1.ReplayMemory && replay.Mode != transportv1alpha1.ReplayDurable {
		...
		if streaming.Routing != nil {
			...
			if strings.TrimSpace(string(routing.Mode)) != "" &&
				routing.Mode != transportv1alpha1.TransportRoutingAuto && routing.Mode != transportv1alpha1.TransportRoutingHub && routing.Mode != transportv1alpha1.TransportRoutingP2P {
			...
			if strings.TrimSpace(string(routing.FanOut)) != "" &&
				routing.FanOut != transportv1alpha1.TransportFanOutSequential && routing.FanOut != transportv1alpha1.TransportFanOutParallel {
		...
		if streaming.Partitioning != nil {
			...
			if strings.TrimSpace(string(partitioning.Mode)) != "" &&
				partitioning.Mode != transportv1alpha1.TransportPartitionNone && partitioning.Mode != transportv1alpha1.TransportPartitionPreserve && partitioning.Mode != transportv1alpha1.TransportPartitionHash {
		...
		if streaming.Lifecycle != nil {
			...
			if strings.TrimSpace(string(lifecycle.Strategy)) != "" &&
				lifecycle.Strategy != transportv1alpha1.TransportUpgradeRolling && lifecycle.Strategy != transportv1alpha1.TransportUpgradeDrainCutover && lifecycle.Strategy != transportv1alpha1.TransportUpgradeBlueGreen {
		...
		if streaming.Recording != nil {
			...
			if strings.TrimSpace(string(recording.Mode)) != "" &&
				recording.Mode != transportv1alpha1.TransportRecordingOff && recording.Mode != transportv1alpha1.TransportRecordingMetadata && recording.Mode != transportv1alpha1.TransportRecordingPayload {
...
func (v *StoryCustomValidator) validateEngramStepCached(
	...
	if !ok {
		...
		if err := reader.Get(ctx, types.NamespacedName{Name: templateName, Namespace: ""}, template); err != nil {
			if apierrors.IsNotFound(err) {
...
func validateStepTemplateRefs(step *bubushv1alpha1.Step, allStepNames map[string]struct{}, reachable map[string]struct{}) []string {
	...
	for _, tpl := range templates {
		stepRefs := templating.ExtractStepReferences(tpl)

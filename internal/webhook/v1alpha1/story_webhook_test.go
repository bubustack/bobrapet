...
var _ = Describe("Story Webhook", func() {
	...
	Context("loop validation", func() {
		It("rejects loop steps missing template refs", func() {
			story := minimalStory("loops")
			story.Spec.Steps[0] = bubushv1alpha1.Step{
				Name: "fanout",
			...
			_, err := validator.ValidateCreate(ctx, story)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("must reference an Engram template"))
		})

		It("rejects loop execution overrides with non-positive values", func() {
			zero := int32(0)
			story := minimalStory("loops")
			story.Spec.Steps[0] = bubushv1alpha1.Step{
				Name: "fanout",
				...
				Execution: &bubushv1alpha1.ExecutionOverrides{
					Loop: &bubushv1alpha1.LoopPolicy{
			...
			_, err := validator.ValidateCreate(ctx, story)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("defaultBatchSize"))
		})
	})
})

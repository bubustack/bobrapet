...
func (e *Evaluator) EvaluateWhenCondition(when string, vars map[string]interface{}) (bool, error) {
	if strings.TrimSpace(when) == "" {
		return true, nil
	}

	program, err := e.compile(context.Background(), when, "when")
	if err != nil {
...
func (e *Evaluator) ResolveWithInputs(with map[string]interface{}, vars map[string]interface{}) (map[string]interface{}, error) {
	if with == nil {
		return nil, nil
	}
	...
	for key, val := range with {
		...
		if strings.HasPrefix(strVal, "{{") && strings.HasSuffix(strVal, "}}") {
			...
			program, err := e.compile(context.Background(), expr, "with")
			if err != nil {

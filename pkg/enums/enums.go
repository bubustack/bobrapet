...
// Pending → Running → {Succeeded|Failed|Finished|Compensated|Paused|Blocked|Scheduling|Timeout|Aborted|Skipped}
//
// Some resources may also support Paused for manual intervention scenarios.
// +kubebuilder:validation:Enum=Pending;Running;Succeeded;Failed;Finished;Canceled;Compensated;Paused;Blocked;Scheduling;Timeout;Aborted;Skipped;
//
// nolint:lll
type Phase string
...
// +kubebuilder:validation:Enum=condition;parallel;sleep;stop;wait;executeStory;gate
//
// nolint:lll
...
const (
	...
	// RealtimePattern executes the story as a persistent pipeline of long-running services.
	// This is suitable for low-latency, real-time workflows.
	RealtimePattern StoryPattern = "realtime"
)

// IsRealtime returns true if the pattern is realtime.
func (p StoryPattern) IsRealtime() bool {
	return p == RealtimePattern
}

...
// ConfigEnvVars returns a slice containing the BUBU_CONFIG env var when
...
func ConfigEnvVars(raw []byte) []corev1.EnvVar {
	...
	return []corev1.EnvVar{
		{Name: contracts.ConfigEnv, Value: string(raw)},
	}
}

// AppendConfigEnvVar appends the BUBU_CONFIG env var to envVars when raw
// is non-empty.
//
// Arguments:
...
func AppendConfigEnvVar(envVars *[]corev1.EnvVar, raw []byte) {
	...
	*envVars = append(*envVars, corev1.EnvVar{Name: contracts.ConfigEnv, Value: string(raw)})
}

...
func TestEnsureServiceIgnoresAPIDefaultedSpecFields(t *testing.T) {
	scheme := runtime.NewScheme()
	require.NoError(t, corev1.AddToScheme(scheme))
	...
	ctx := context.Background()
	_, result, err := EnsureService(ctx, cl, scheme, owner, desired)
	require.NoError(t, err)
	require.Equal(t, controllerutil.OperationResultCreated, result)
	...
	stored.Spec.ClusterIP = "10.0.0.1"
	stored.Spec.ClusterIPs = []string{"10.0.0.1"}
	stored.Spec.Type = corev1.ServiceTypeClusterIP
	stored.Spec.SessionAffinity = corev1.ServiceAffinityNone
	policy := corev1.ServiceInternalTrafficPolicyCluster
	stored.Spec.InternalTrafficPolicy = &policy
	stored.Spec.Ports[0].Protocol = corev1.ProtocolTCP
	require.NoError(t, cl.Update(ctx, stored))

	ensured, secondResult, err := EnsureService(ctx, cl, scheme, owner, desired.DeepCopy())
	require.NoError(t, err)
	require.Equal(t, controllerutil.OperationResultNone, secondResult)
	require.Equal(t, "10.0.0.1", ensured.Spec.ClusterIP)
	require.Equal(t, corev1.ProtocolTCP, ensured.Spec.Ports[0].Protocol)
}

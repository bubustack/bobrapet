...
func PatchStoryRunPhase(
	...
	return PatchStoryRunPhaseWithError(ctx, k8sClient, srun, phase, reason, message, nil)
}

// PatchStoryRunPhaseWithError updates StoryRun phase and persists the supplied
// structured status error in the same status patch when provided.
//
...
func PatchStoryRunPhaseWithError(
	...
	reason string,
	message string,
	statusErr *k8sruntime.RawExtension,
) error {
	return kubeutil.RetryableStatusPatch(ctx, k8sClient, srun, func(obj client.Object) {
		...
		if statusErr != nil {
			sr.Status.Error = &k8sruntime.RawExtension{Raw: append([]byte(nil), statusErr.Raw...)}
		}
...
func BuildStoryRunError(reason, message string, failedSteps map[string]bool, stepStates map[string]runsv1alpha1.StepState) *k8sruntime.RawExtension { //nolint:lll

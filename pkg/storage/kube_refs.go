...
func parseKubeRefString(raw string) (*kubeRef, error) {
	...
	if err := enforceWorkloadNamespace(ns); err != nil {
		return nil, err
	}
	if name == "" {
...
func parseKubeRefMap(raw map[string]any) (*kubeRef, error) {
	...
	if err := enforceWorkloadNamespace(namespace); err != nil {
		return nil, err
	}
	if namespace == "" {
...
func enforceWorkloadNamespace(namespace string) error {
	namespace = strings.TrimSpace(namespace)
	if namespace == "" {
		return nil
	}
	workloadNamespace := defaultNamespace()
	if workloadNamespace == "" {
		return fmt.Errorf("explicit namespace %q is not allowed without workload namespace context", namespace)
	}
	if namespace != workloadNamespace {
		return fmt.Errorf("cross-namespace references are not allowed: %q (workload namespace: %q)", namespace, workloadNamespace) //nolint:lll
	}
	return nil
}
...
func decodeRefValue(raw []byte, format, ref string) (any, error) {
	...
	case "raw": //nolint:goconst // format literal
		return string(raw), nil
	case "json": //nolint:goconst // format literal
		var decoded any

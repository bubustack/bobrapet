...
func TestStorageManager_HydrateStorageRefsOnly(t *testing.T) {
	ctx := context.Background()
	mock := newMockStore()
	sm := &StorageManager{
		store:         mock,
	...
	t.Run("hydrates storage refs but leaves nested kube refs untouched", func(t *testing.T) {
		got, err := sm.HydrateStorageRefsOnly(ctx, map[string]any{
			"$bubuStorageRef": "test/path.json",
		})
	...
	t.Run("passes direct kube refs through unchanged", func(t *testing.T) {
		input := map[string]any{
			"$bubuSecretRef": "kube-system/cluster-creds:token",
		}
		got, err := sm.HydrateStorageRefsOnly(ctx, input)
		require.NoError(t, err)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantOffload {
				...
				if _, hasType := refMap["$bubuStorageContentType"]; !hasType {
					t.Fatal("Expected storage content type in result")
				}
			}
...
func TestStorageManager_Dehydrate_NestedMap(t *testing.T) {
	...
	if largeRef, ok := resultMap["large"].(map[string]any); ok {
		...
		if _, hasType := largeRef["$bubuStorageContentType"]; !hasType {
			t.Error("Large value should include storage content type")
		}
	} else {
...
func TestStorageManager_Dehydrate_SliceElements(t *testing.T) {
	...
	if _, ok := ref1["$bubuStorageContentType"]; !ok {
		t.Errorf("Expected content type metadata on second element ref, got %v", dehydratedSlice[1])
	}
	...
	if _, ok := nestedRef["$bubuStorageContentType"]; !ok {
		t.Errorf("Expected content type metadata on nested ref, got %v", mapWithRef["key"])
	}

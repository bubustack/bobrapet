...
func MergeSettingsWithStreaming(base *runtime.RawExtension, streaming any) (*runtime.RawExtension, error) {
	...
	return kubeutil.MergeWithBlocks(base, streamingRaw)
}
...
func applyStreamingDefaults(streaming any) any {
	...
	case *bubuv1alpha1.TransportStreamingSettings:
		return applyStoryStreamingDefaults(typed)
	default:
		return streaming
	}
}

func applyTransportStreamingDefaults(in *transportv1alpha1.TransportStreamingSettings) *transportv1alpha1.TransportStreamingSettings {
	if in == nil {
...
func applyStoryStreamingDefaults(in *bubuv1alpha1.TransportStreamingSettings) *bubuv1alpha1.TransportStreamingSettings {
	if in == nil {
		return nil
	}
	out := in.DeepCopy()
	applyFlowControlDefaultsStory(out)
	applyRoutingDefaultsStory(out)
	applyLaneDefaultsStory(out)
	return out
}
...
func applyFlowControlDefaultsStory(settings *bubuv1alpha1.TransportStreamingSettings) {
	if settings == nil {
		return
	}
	fc := settings.FlowControl
	if fc == nil {
		fc = &bubuv1alpha1.TransportFlowControlSettings{}
		settings.FlowControl = fc
	}
	if shouldDefaultFlowControlStory(fc) {
		fc.Mode = bubuv1alpha1.FlowControlCredits
		fc.AckEvery = &bubuv1alpha1.TransportFlowAckSettings{
			Messages: int32Ptr(10),
			MaxDelay: stringPtr("250ms"),
		}
		fc.PauseThreshold = &bubuv1alpha1.TransportFlowThreshold{BufferPct: int32Ptr(90)}
		fc.ResumeThreshold = &bubuv1alpha1.TransportFlowThreshold{BufferPct: int32Ptr(50)}
		return
	}
	if fc.Mode == bubuv1alpha1.FlowControlCredits || fc.Mode == bubuv1alpha1.FlowControlWindow {
		if fc.AckEvery == nil {
			fc.AckEvery = &bubuv1alpha1.TransportFlowAckSettings{Messages: int32Ptr(10), MaxDelay: stringPtr("250ms")}
		}
		if fc.PauseThreshold == nil {
			fc.PauseThreshold = &bubuv1alpha1.TransportFlowThreshold{BufferPct: int32Ptr(90)}
		}
...
func applyRoutingDefaultsStory(settings *bubuv1alpha1.TransportStreamingSettings) {
	if settings == nil {
		return
	}
	if settings.Routing == nil {
		settings.Routing = &bubuv1alpha1.TransportRoutingSettings{}
	}
	if settings.Routing.MaxDownstreams == nil {
		settings.Routing.MaxDownstreams = int32Ptr(32)
	}
...
func applyLaneDefaultsStory(settings *bubuv1alpha1.TransportStreamingSettings) {
	if settings == nil {
		return
	}
	if len(settings.Lanes) == 0 {
	...
	for i := range settings.Lanes {
		applyLaneDefaultsStoryEntry(&settings.Lanes[i])
	}
}

func applyLaneDefaultsStoryEntry(lane *bubuv1alpha1.TransportLane) {
	if lane == nil {
		return
	}
	if lane.MaxMessages == nil {
	...
	if lane.Direction == "" {
		lane.Direction = bubuv1alpha1.TransportLaneBidirectional
	}
}
...
func shouldDefaultFlowControlStory(fc *bubuv1alpha1.TransportFlowControlSettings) bool {
	if fc == nil {
		return true
	}
	if fc.Mode != "" {

...
func validateTransportSpecWithReason(path *field.Path, spec *transportv1alpha1.TransportSpec) *ValidationResult {
	...
	streamingResult := validateStreamingSettings(path.Child("streaming"), spec.Streaming)
	result.Errors = append(result.Errors, streamingResult.Errors...)
	if result.PrimaryReason == "" && streamingResult.PrimaryReason != "" {
		result.PrimaryReason = streamingResult.PrimaryReason
	}
